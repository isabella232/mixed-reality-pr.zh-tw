...
如此一來，就可以將 [操作條件約束元件](ConstraintManager.md#transform-constraints) 加入至物件，以限制操作。 這些是使用操作的特殊元件，並以某種方式變更操作行為。
...
- 旋轉大約取得點 *：使用手旋轉旋轉物件，但指標光線點擊點的指標。 適用于檢查。
...
這種設定會啟用連結的 [條件約束管理員](ConstraintManager.md)。 轉換變更將由註冊至所選 [條件約束管理員](ConstraintManager.md)的條件約束處理。

#### <a name="constraint-manager"></a>條件約束管理員

下拉式清單可讓您選取任何附加的 [條件約束管理員](ConstraintManager.md)。 物件操作工具可確保所有時間都有附加的 [條件約束管理員](ConstraintManager.md) 。
