...
此資料提供者 **不會使用 unity** 的 **XR 管線** 或 **舊版 XR 管線**，但由於 Unity 的 XR 管線會 handeled 控制器和 Headtracking，因此必須遵循為 **Oculus 執行設定專案** 的步驟，以確保您使用的是 **XR 管線** ，而不是即將淘汰的 **舊版 XR 管線**。
...
1. 流覽至混合現實工具組 > Oculus > 個別的 Oculus Integration Unity 模組  ![ OculusSeparationAsmdef](../Images/CrossPlatform/OculusQuest/OculusSeparationAsmdef.png)
