...
### <a name="unity-asset-unitypackage-files"></a><span data-ttu-id="11221-116">Unity 資產 (. unitypackage) 檔案</span><span class="sxs-lookup"><span data-stu-id="11221-116">Unity asset (.unitypackage) files</span></span>
...
1. <span data-ttu-id="11221-140">關閉 Unity 並刪除連結 **庫** 資料夾 (先閱讀以下的附注！ ) 。</span><span class="sxs-lookup"><span data-stu-id="11221-140">Close Unity and delete the **Library** folder (read the note below first!).</span></span> <span data-ttu-id="11221-141">必須執行此步驟，才能強制 Unity 重新整理其資產資料庫，並協調現有的自訂設定檔。</span><span class="sxs-lookup"><span data-stu-id="11221-141">This step is necessary to force Unity to refresh its asset database and reconcile existing custom profiles.</span></span>
...
<span data-ttu-id="11221-158">[資料夾重新命名](#folder-renames-in-240) 
...
### <a name="eye-gaze-pointer-behavior-in-240"></a><span data-ttu-id="11221-186">2.4.0 中的眼睛指標行為</span><span class="sxs-lookup"><span data-stu-id="11221-186">Eye gaze pointer behavior in 2.4.0</span></span>
...
<span data-ttu-id="11221-200">現在就完成了 .。。</span><span class="sxs-lookup"><span data-stu-id="11221-200">Do this now...</span></span>
...
<span data-ttu-id="11221-239">雖然現有的程式碼在版本2.3.0 中仍可繼續運作，但建議您更新至新的類別。</span><span class="sxs-lookup"><span data-stu-id="11221-239">While existing code will continue to function in version 2.3.0, it is recommended to update to the new class.</span></span>
...
<span data-ttu-id="11221-260">應用程式程式碼可能會因為這種變更而需要更新。</span><span class="sxs-lookup"><span data-stu-id="11221-260">Application code may require updating due to this change.</span></span>
...
- [<span data-ttu-id="11221-276">API 變更</span><span class="sxs-lookup"><span data-stu-id="11221-276">API changes</span></span>](#api-changes-in-210)
...
<span data-ttu-id="11221-294">**互動**</span><span class="sxs-lookup"><span data-stu-id="11221-294">**Interactable**</span></span>
...
<span data-ttu-id="11221-324">如需這些變更的詳細資訊，請參閱「 [輸入模擬服務](../features/input-simulation/InputSimulationService.md) 」一文。</span><span class="sxs-lookup"><span data-stu-id="11221-324">For more information on these changes, please see the [input simulation service](../features/input-simulation/InputSimulationService.md) article.</span></span>
...
<span data-ttu-id="11221-341">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="11221-341">**MixedRealityToolkit**</span></span>
...
<span data-ttu-id="11221-351">使用新的 CoreSystem 類別可確保您的應用程式程式碼在您將應用程式變更為使用不同的服務註冊機構時，不需要更新 (例如：其中一個實驗性服務管理員) 。</span><span class="sxs-lookup"><span data-stu-id="11221-351">Using the new CoreSystem class will ensure that your application code will not need updating if you change the application to use a different service registrar (ex: one of the experimental service managers).</span></span>
...
<span data-ttu-id="11221-370">新 `RegisterHandler` 的 API 和 `UnregisterHandler` ：</span><span class="sxs-lookup"><span data-stu-id="11221-370">New API `RegisterHandler` and `UnregisterHandler`:</span></span>
...
<span data-ttu-id="11221-383">IMixedRealitySpatialAwarenessSystem 和 IMixedRealitySpatialAwarenessObserver 介面採用多個中斷變更，如下所述。</span><span class="sxs-lookup"><span data-stu-id="11221-383">The IMixedRealitySpatialAwarenessSystem and IMixedRealitySpatialAwarenessObserver interfaces have taken multiple breaking changes as described below.</span></span>
...
<span data-ttu-id="11221-401">**_SurfaceMagnetism_**</span><span class="sxs-lookup"><span data-stu-id="11221-401">**_SurfaceMagnetism_**</span></span>
...
- <span data-ttu-id="11221-421">BaseNearInteractionTouchable 已重新命名為 PokeThreshold，因此它會清楚指出 PokeThreshold 是 DebounceThreshold 的對應項。</span><span class="sxs-lookup"><span data-stu-id="11221-421">BaseNearInteractionTouchable.DistBack has been renamed to PokeThreshold This makes it clear that PokeThreshold is the counterpart to DebounceThreshold.</span></span> <span data-ttu-id="11221-422">當超過 PokeThreshold 時，即會啟動可觸式，並在超過 DebounceThreshold 時釋放。</span><span class="sxs-lookup"><span data-stu-id="11221-422">A touchable is activated when the PokeThreshold is crossed, and released when DebounceThreshold is crossed.</span></span>
...
<span data-ttu-id="11221-436">**HandInteractionPanZoom**</span><span class="sxs-lookup"><span data-stu-id="11221-436">**HandInteractionPanZoom**</span></span>
...
<span data-ttu-id="11221-440">在2.0.0 版本中，所有的官方混合現實工具組元件名稱及其相關聯的元件定義 (. asmdef) 檔案都已更新為符合下列模式。</span><span class="sxs-lookup"><span data-stu-id="11221-440">In The 2.0.0 release, all of the official Mixed Reality Toolkit assembly names and their associated assembly definition (.asmdef) files have been updated to fit the following pattern.</span></span>

```c#
Microsoft.MixedReality.Toolkit[.<name>]
```

<span data-ttu-id="11221-441">在某些情況下，已合併多個元件以建立更好的 unity 內容。</span><span class="sxs-lookup"><span data-stu-id="11221-441">In some instances, multiple assemblies have been merged to create better unity of their contents.</span></span> <span data-ttu-id="11221-442">如果您的專案使用自訂的 asmdef 檔案，則可能需要更新。</span><span class="sxs-lookup"><span data-stu-id="11221-442">If your project uses custom .asmdef files, they may require updating.</span></span>

<span data-ttu-id="11221-443">下表說明 RC2 asmdef 檔案名如何對應至2.0.0 版本。</span><span class="sxs-lookup"><span data-stu-id="11221-443">The following tables describe how the RC2 .asmdef file names map to the 2.0.0 release.</span></span> <span data-ttu-id="11221-444">所有元件名稱都符合 asmdef 檔案名。</span><span class="sxs-lookup"><span data-stu-id="11221-444">All assembly names match the .asmdef file name.</span></span>

<span data-ttu-id="11221-445">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="11221-445">**MixedRealityToolkit**</span></span>

| <span data-ttu-id="11221-446">RC2</span><span class="sxs-lookup"><span data-stu-id="11221-446">RC2</span></span> | <span data-ttu-id="11221-447">2.0.0</span><span class="sxs-lookup"><span data-stu-id="11221-447">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="11221-448">MixedRealityToolkit.asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-448">MixedRealityToolkit.asmdef</span></span> | <span data-ttu-id="11221-449">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-449">Microsoft.MixedReality.Toolkit.asmdef</span></span> |
| <span data-ttu-id="11221-450">MixedRealityToolkit. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-450">MixedRealityToolkit.Core.BuildAndDeploy.asmdef</span></span> | <span data-ttu-id="11221-451">MixedReality. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-451">Microsoft.MixedReality.Toolkit.Editor.BuildAndDeploy.asmdef</span></span> |
| <span data-ttu-id="11221-452">MixedRealityToolkit，asmdef 的</span><span class="sxs-lookup"><span data-stu-id="11221-452">MixedRealityToolkit.Core.Definitions.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="11221-453">已移除，請使用 MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-453">Removed, use Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="11221-454">MixedRealityToolkit. EditorClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-454">MixedRealityToolkit.Core.Extensions.EditorClassExtensions.asmdef</span></span> | <span data-ttu-id="11221-455">MixedReality. ClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-455">Microsoft.MixedReality.Toolkit.Editor.ClassExtensions.asmdef</span></span>
| <span data-ttu-id="11221-456">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-456">MixedRealityToolkit.Core.Inspectors.asmdef</span></span> | <span data-ttu-id="11221-457">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-457">Microsoft.MixedReality.Toolkit.Editor.Inspectors.asmdef</span></span> |
| <span data-ttu-id="11221-458">MixedRealityToolkit ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-458">MixedRealityToolkit.Core.Inspectors.ServiceInspectors.asmdef</span></span> | <span data-ttu-id="11221-459">MixedReality. ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-459">Microsoft.MixedReality.Toolkit.Editor.ServiceInspectors.asmdef</span></span> |
| <span data-ttu-id="11221-460">MixedRealityToolkit. UtilitiesAsync. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-460">MixedRealityToolkit.Core.UtilitiesAsync.asmdef</span></span> | <span data-ttu-id="11221-461">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-461">Microsoft.MixedReality.Toolkit.Async.asmdef</span></span> |
| <span data-ttu-id="11221-462">Asmdef 的 MixedRealityToolkit。</span><span class="sxs-lookup"><span data-stu-id="11221-462">MixedRealityToolkit.Core.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="11221-463">MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-463">Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="11221-464">MixedRealityToolkit Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-464">MixedRealityToolkit.Utilities.Gltf.asmdef</span></span> | <span data-ttu-id="11221-465">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-465">Microsoft.MixedReality.Toolkit.Gltf.asmdef</span></span> |
| <span data-ttu-id="11221-466">MixedRealityToolkit，Gltf asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-466">MixedRealityToolkit.Utilities.Gltf.Importers.asmdef</span></span> | <span data-ttu-id="11221-467">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-467">Microsoft.MixedReality.Toolkit.Gltf.Importers.asmdef</span></span> |

<span data-ttu-id="11221-468">**MixedRealityToolkit。提供者**</span><span class="sxs-lookup"><span data-stu-id="11221-468">**MixedRealityToolkit.Providers**</span></span>

| <span data-ttu-id="11221-469">RC2</span><span class="sxs-lookup"><span data-stu-id="11221-469">RC2</span></span> | <span data-ttu-id="11221-470">2.0.0</span><span class="sxs-lookup"><span data-stu-id="11221-470">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="11221-471">MixedRealityToolkit. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-471">MixedRealityToolkit.Providers.OpenVR.asmdef</span></span> | <span data-ttu-id="11221-472">MixedReality. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-472">Microsoft.MixedReality.Toolkit.Providers.OpenVR.asmdef</span></span> |
| <span data-ttu-id="11221-473">MixedRealityToolkit. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-473">MixedRealityToolkit.Providers.WindowsMixedReality.asmdef</span></span> | <span data-ttu-id="11221-474">MixedReality. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-474">Microsoft.MixedReality.Toolkit.Providers.WindowsMixedReality.asmdef</span></span> |
| <span data-ttu-id="11221-475">MixedRealityToolkit. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-475">MixedRealityToolkit.Providers.WindowsVoiceInput.asmdef</span></span> | <span data-ttu-id="11221-476">MixedReality. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-476">Microsoft.MixedReality.Toolkit.Providers.WindowsVoiceInput.asmdef</span></span> |

<span data-ttu-id="11221-477">**MixedRealityToolkit 服務**</span><span class="sxs-lookup"><span data-stu-id="11221-477">**MixedRealityToolkit.Services**</span></span>

| <span data-ttu-id="11221-478">RC2</span><span class="sxs-lookup"><span data-stu-id="11221-478">RC2</span></span> | <span data-ttu-id="11221-479">2.0.0</span><span class="sxs-lookup"><span data-stu-id="11221-479">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="11221-480">MixedRealityToolkit. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-480">MixedRealityToolkit.Services.BoundarySystem.asmdef</span></span> | <span data-ttu-id="11221-481">MixedReality. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-481">Microsoft.MixedReality.Toolkit.Services.BoundarySystem.asmdef</span></span> |
| <span data-ttu-id="11221-482">MixedRealityToolkit. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-482">MixedRealityToolkit.Services.CameraSystem.asmdef</span></span> | <span data-ttu-id="11221-483">MixedReality. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-483">Microsoft.MixedReality.Toolkit.Services.CameraSystem.asmdef</span></span> |
| <span data-ttu-id="11221-484">MixedRealityToolkit. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-484">MixedRealityToolkit.Services.DiagnosticsSystem.asmdef</span></span> | <span data-ttu-id="11221-485">MixedReality. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-485">Microsoft.MixedReality.Toolkit.Services.DiagnosticsSystem.asmdef</span></span> |
| <span data-ttu-id="11221-486">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-486">MixedRealityToolkit.Services.InputSimulation.asmdef</span></span> | <span data-ttu-id="11221-487">MixedReality. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-487">Microsoft.MixedReality.Toolkit.Services.InputSimulation.asmdef</span></span> |
| <span data-ttu-id="11221-488">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-488">MixedRealityToolkit.Services.InputSimulation.Editor.asmdef</span></span> | <span data-ttu-id="11221-489">MixedReality. InputSimulation. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-489">Microsoft.MixedReality.Toolkit.Services.InputSimulation.Editor.asmdef</span></span> |
| <span data-ttu-id="11221-490">MixedRealityToolkit. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-490">MixedRealityToolkit.Services.InputSystem.asmdef</span></span> | <span data-ttu-id="11221-491">MixedReality. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-491">Microsoft.MixedReality.Toolkit.Services.InputSystem.asmdef</span></span> |
| <span data-ttu-id="11221-492">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-492">MixedRealityToolkit.Services.Inspectors.asmdef</span></span> | <span data-ttu-id="11221-493">MixedReality. InputSystem. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-493">Microsoft.MixedReality.Toolkit.Services.InputSystem.Editor.asmdef</span></span> |
| <span data-ttu-id="11221-494">MixedRealityToolkit. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-494">MixedRealityToolkit.Services.SceneSystem.asmdef</span></span> | <span data-ttu-id="11221-495">MixedReality. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-495">Microsoft.MixedReality.Toolkit.Services.SceneSystem.asmdef</span></span> |
| <span data-ttu-id="11221-496">MixedRealityToolkit. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-496">MixedRealityToolkit.Services.SpatialAwarenessSystem.asmdef</span></span> | <span data-ttu-id="11221-497">MixedReality. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-497">Microsoft.MixedReality.Toolkit.Services.SpatialAwarenessSystem.asmdef</span></span> |
| <span data-ttu-id="11221-498">MixedRealityToolkit. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-498">MixedRealityToolkit.Services.TeleportSystem.asmdef</span></span> | <span data-ttu-id="11221-499">MixedReality. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-499">Microsoft.MixedReality.Toolkit.Services.TeleportSystem.asmdef</span></span> |

<span data-ttu-id="11221-500">**MixedRealityToolkit SDK**</span><span class="sxs-lookup"><span data-stu-id="11221-500">**MixedRealityToolkit.SDK**</span></span>

| <span data-ttu-id="11221-501">RC2</span><span class="sxs-lookup"><span data-stu-id="11221-501">RC2</span></span> | <span data-ttu-id="11221-502">2.0.0</span><span class="sxs-lookup"><span data-stu-id="11221-502">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="11221-503">MixedRealityToolkit. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-503">MixedRealityToolkit.SDK.asmdef</span></span> | <span data-ttu-id="11221-504">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-504">Microsoft.MixedReality.Toolkit.SDK.asmdef</span></span> |
| <span data-ttu-id="11221-505">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-505">MixedRealityToolkit.SDK.Inspectors.asmdef</span></span> | <span data-ttu-id="11221-506">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-506">Microsoft.MixedReality.Toolkit.SDK.Inspectors.asmdef</span></span> |

<span data-ttu-id="11221-507">**MixedRealityToolkit 範例**</span><span class="sxs-lookup"><span data-stu-id="11221-507">**MixedRealityToolkit.Examples**</span></span>

| <span data-ttu-id="11221-508">RC2</span><span class="sxs-lookup"><span data-stu-id="11221-508">RC2</span></span> | <span data-ttu-id="11221-509">2.0.0</span><span class="sxs-lookup"><span data-stu-id="11221-509">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="11221-510">MixedRealityToolkit。例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-510">MixedRealityToolkit.Examples.asmdef</span></span> | <span data-ttu-id="11221-511">MixedReality，例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-511">Microsoft.MixedReality.Toolkit.Examples.asmdef</span></span> |
| <span data-ttu-id="11221-512">MixedRealityToolkit. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-512">MixedRealityToolkit.Examples.Demos.Gltf.asmdef</span></span> | <span data-ttu-id="11221-513">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-513">Microsoft.MixedReality.Toolkit.Demos.Gltf.asmdef</span></span> |
| <span data-ttu-id="11221-514">MixedRealityToolkit，例如 StandardShader asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-514">MixedRealityToolkit.Examples.Demos.StandardShader.Inspectors.asmdef</span></span> | <span data-ttu-id="11221-515">MixedReality. StandardShader. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-515">Microsoft.MixedReality.Toolkit.Demos.StandardShader.Inspectors.asmdef</span></span> |
| <span data-ttu-id="11221-516">MixedRealityToolkit，InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-516">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.asmdef</span></span> | <span data-ttu-id="11221-517">MixedReality. InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-517">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.asmdef</span></span> |
| <span data-ttu-id="11221-518">MixedRealityToolkit，例如 InspectorFields asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-518">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.Inspectors.asmdef</span></span> | <span data-ttu-id="11221-519">MixedReality. InspectorFields. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-519">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.Inspectors.asmdef</span></span> |
| <span data-ttu-id="11221-520">MixedRealityToolkit，例如 Interactables. asmdef</span><span class="sxs-lookup"><span data-stu-id="11221-520">MixedRealityToolkit.Examples.Demos.UX.Interactables.asmdef</span></span> | <span data-ttu-id="11221-521">MixedReality. Interactables. asmdef。</span><span class="sxs-lookup"><span data-stu-id="11221-521">Microsoft.MixedReality.Toolkit.Demos.UX.Interactables.asmdef</span></span> |
