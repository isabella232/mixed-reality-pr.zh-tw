...
### <a name="unity-asset-unitypackage-files"></a><span data-ttu-id="1e0f7-116">Unity 資產 (. unitypackage) 檔案</span><span class="sxs-lookup"><span data-stu-id="1e0f7-116">Unity asset (.unitypackage) files</span></span>
...
1. <span data-ttu-id="1e0f7-140">關閉 Unity 並刪除連結 **庫** 資料夾 (先閱讀以下的附注！ ) 。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-140">Close Unity and delete the **Library** folder (read the note below first!).</span></span> <span data-ttu-id="1e0f7-141">必須執行此步驟，才能強制 Unity 重新整理其資產資料庫，並協調現有的自訂設定檔。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-141">This step is necessary to force Unity to refresh its asset database and reconcile existing custom profiles.</span></span>
...
<span data-ttu-id="1e0f7-158">[資料夾重新命名](#folder-renames-in-240) 
...
### <a name="eye-gaze-pointer-behavior-in-240"></a><span data-ttu-id="1e0f7-186">2.4.0 中的眼睛指標行為</span><span class="sxs-lookup"><span data-stu-id="1e0f7-186">Eye gaze pointer behavior in 2.4.0</span></span>
...
<span data-ttu-id="1e0f7-200">現在就完成了 .。。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-200">Do this now...</span></span>
...
<span data-ttu-id="1e0f7-239">雖然現有的程式碼在版本2.3.0 中仍可繼續運作，但建議您更新至新的類別。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-239">While existing code will continue to function in version 2.3.0, it is recommended to update to the new class.</span></span>
...
<span data-ttu-id="1e0f7-260">應用程式程式碼可能會因為這種變更而需要更新。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-260">Application code may require updating due to this change.</span></span>
...
- [<span data-ttu-id="1e0f7-276">API 變更</span><span class="sxs-lookup"><span data-stu-id="1e0f7-276">API changes</span></span>](#api-changes-in-210)
...
<span data-ttu-id="1e0f7-294">**互動**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-294">**Interactable**</span></span>
...
<span data-ttu-id="1e0f7-324">如需這些變更的詳細資訊，請參閱「 [輸入模擬服務](../features/input-simulation/InputSimulationService.md) 」一文。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-324">For more information on these changes, please see the [input simulation service](../features/input-simulation/InputSimulationService.md) article.</span></span>
...
<span data-ttu-id="1e0f7-341">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-341">**MixedRealityToolkit**</span></span>
...
<span data-ttu-id="1e0f7-351">使用新的 CoreSystem 類別可確保您的應用程式程式碼在您將應用程式變更為使用不同的服務註冊機構時，不需要更新 (例如：其中一個實驗性服務管理員) 。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-351">Using the new CoreSystem class will ensure that your application code will not need updating if you change the application to use a different service registrar (ex: one of the experimental service managers).</span></span>
...
<span data-ttu-id="1e0f7-370">新 `RegisterHandler` 的 API 和 `UnregisterHandler` ：</span><span class="sxs-lookup"><span data-stu-id="1e0f7-370">New API `RegisterHandler` and `UnregisterHandler`:</span></span>
...
<span data-ttu-id="1e0f7-383">IMixedRealitySpatialAwarenessSystem 和 IMixedRealitySpatialAwarenessObserver 介面採用多個中斷變更，如下所述。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-383">The IMixedRealitySpatialAwarenessSystem and IMixedRealitySpatialAwarenessObserver interfaces have taken multiple breaking changes as described below.</span></span>
...
<span data-ttu-id="1e0f7-401">**_SurfaceMagnetism_**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-401">**_SurfaceMagnetism_**</span></span>
...
- <span data-ttu-id="1e0f7-421">BaseNearInteractionTouchable 已重新命名為 PokeThreshold，因此它會清楚指出 PokeThreshold 是 DebounceThreshold 的對應項。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-421">BaseNearInteractionTouchable.DistBack has been renamed to PokeThreshold This makes it clear that PokeThreshold is the counterpart to DebounceThreshold.</span></span> <span data-ttu-id="1e0f7-422">當超過 PokeThreshold 時，即會啟動可觸式，並在超過 DebounceThreshold 時釋放。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-422">A touchable is activated when the PokeThreshold is crossed, and released when DebounceThreshold is crossed.</span></span>

<span data-ttu-id="1e0f7-423">**ReadOnlyAttribute**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-423">**ReadOnlyAttribute**</span></span>

<span data-ttu-id="1e0f7-424">已將 `Microsoft.MixedReality.Toolkit` 命名空間加入至 `ReadOnlyAttribute` 、 `BeginReadOnlyGroupAttribute` 和 `EndReadOnlyGroupAttribute` 。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-424">The `Microsoft.MixedReality.Toolkit` namespace has been added to `ReadOnlyAttribute`, `BeginReadOnlyGroupAttribute`, and `EndReadOnlyGroupAttribute`.</span></span>

<span data-ttu-id="1e0f7-425">**PointerClickHandler**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-425">**PointerClickHandler**</span></span>

<span data-ttu-id="1e0f7-426">`PointerClickHandler` 類別已被取代。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-426">The `PointerClickHandler` class has been deprecated.</span></span> <span data-ttu-id="1e0f7-427">您 `PointerHandler` 應該改為使用，它會提供相同的功能。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-427">The `PointerHandler` should be used instead, it provides the same functionality.</span></span>

<span data-ttu-id="1e0f7-428">**HoloLens clicker 支援**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-428">**HoloLens clicker support**</span></span>

<span data-ttu-id="1e0f7-429">HoloLens clicker 的控制器對應已從狀況變更 [`WindowsMixedRealityController`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityController) 為狀況 [`WindowsMixedRealityGGVHand`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityGGVHand) 。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-429">The HoloLens clicker's controller mappings have changed from being an unhanded [`WindowsMixedRealityController`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityController) to being an unhanded [`WindowsMixedRealityGGVHand`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityGGVHand).</span></span> <span data-ttu-id="1e0f7-430">為了因應此情況，當您第一次開啟 ControllerMapping 設定檔時，會執行自動更新程式。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-430">To account for this, an automatic updater will run the first time you open your ControllerMapping profile.</span></span> <span data-ttu-id="1e0f7-431">請至少在升級至2.0.0 之後開啟任何自訂設定檔一次，以觸發這個一次性的遷移步驟。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-431">Please open any custom profiles at least once after upgrading to 2.0.0 in order to trigger this one-time migration step.</span></span>

<span data-ttu-id="1e0f7-432">**InteractableHighlight**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-432">**InteractableHighlight**</span></span>

<span data-ttu-id="1e0f7-433">`InteractableHighlight` 類別已被取代。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-433">The `InteractableHighlight` class has been deprecated.</span></span> <span data-ttu-id="1e0f7-434">`InteractableOnFocus` `FocusInteractableStates` 應改為使用類別和資產。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-434">The `InteractableOnFocus` class and `FocusInteractableStates` asset should be used instead.</span></span> <span data-ttu-id="1e0f7-435">若要為建立新的 `Theme` 資產 `InteractableOnFocus` ，請在 [專案] 視窗中按一下滑鼠右鍵，然後選取 [*建立*  >  *混合現實工具* 組  >  *互動*  >  *主題*]。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-435">To create a new `Theme` asset for the `InteractableOnFocus`, right click in the project window and select *Create* > *Mixed Reality Toolkit* > *Interactable* > *Theme*.</span></span>

<span data-ttu-id="1e0f7-436">**HandInteractionPanZoom**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-436">**HandInteractionPanZoom**</span></span>

<span data-ttu-id="1e0f7-437">`HandInteractionPanZoom` 已移至 UI 命名空間，因為它不是輸入元件。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-437">`HandInteractionPanZoom` has been moved to the UI namespace as it was not an input component.</span></span> <span data-ttu-id="1e0f7-438">`HandPanEventData` 也已移至此命名空間，並簡化為與其他 UI 事件資料對應。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-438">`HandPanEventData` has also been moved into this namespace, and simplified to correspond with other UI event data.</span></span>

### <a name="assembly-name-changes-in-200"></a><span data-ttu-id="1e0f7-439">2.0.0 中的元件名稱變更</span><span class="sxs-lookup"><span data-stu-id="1e0f7-439">Assembly name changes in 2.0.0</span></span>

<span data-ttu-id="1e0f7-440">在2.0.0 版本中，所有的官方混合現實工具組元件名稱及其相關聯的元件定義 (. asmdef) 檔案都已更新為符合下列模式。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-440">In The 2.0.0 release, all of the official Mixed Reality Toolkit assembly names and their associated assembly definition (.asmdef) files have been updated to fit the following pattern.</span></span>

```c#
Microsoft.MixedReality.Toolkit[.<name>]
```

<span data-ttu-id="1e0f7-441">在某些情況下，已合併多個元件以建立更好的 unity 內容。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-441">In some instances, multiple assemblies have been merged to create better unity of their contents.</span></span> <span data-ttu-id="1e0f7-442">如果您的專案使用自訂的 asmdef 檔案，則可能需要更新。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-442">If your project uses custom .asmdef files, they may require updating.</span></span>

<span data-ttu-id="1e0f7-443">下表說明 RC2 asmdef 檔案名如何對應至2.0.0 版本。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-443">The following tables describe how the RC2 .asmdef file names map to the 2.0.0 release.</span></span> <span data-ttu-id="1e0f7-444">所有元件名稱都符合 asmdef 檔案名。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-444">All assembly names match the .asmdef file name.</span></span>

<span data-ttu-id="1e0f7-445">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-445">**MixedRealityToolkit**</span></span>

| <span data-ttu-id="1e0f7-446">RC2</span><span class="sxs-lookup"><span data-stu-id="1e0f7-446">RC2</span></span> | <span data-ttu-id="1e0f7-447">2.0.0</span><span class="sxs-lookup"><span data-stu-id="1e0f7-447">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="1e0f7-448">MixedRealityToolkit.asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-448">MixedRealityToolkit.asmdef</span></span> | <span data-ttu-id="1e0f7-449">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-449">Microsoft.MixedReality.Toolkit.asmdef</span></span> |
| <span data-ttu-id="1e0f7-450">MixedRealityToolkit. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-450">MixedRealityToolkit.Core.BuildAndDeploy.asmdef</span></span> | <span data-ttu-id="1e0f7-451">MixedReality. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-451">Microsoft.MixedReality.Toolkit.Editor.BuildAndDeploy.asmdef</span></span> |
| <span data-ttu-id="1e0f7-452">MixedRealityToolkit，asmdef 的</span><span class="sxs-lookup"><span data-stu-id="1e0f7-452">MixedRealityToolkit.Core.Definitions.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="1e0f7-453">已移除，請使用 MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-453">Removed, use Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="1e0f7-454">MixedRealityToolkit. EditorClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-454">MixedRealityToolkit.Core.Extensions.EditorClassExtensions.asmdef</span></span> | <span data-ttu-id="1e0f7-455">MixedReality. ClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-455">Microsoft.MixedReality.Toolkit.Editor.ClassExtensions.asmdef</span></span>
| <span data-ttu-id="1e0f7-456">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-456">MixedRealityToolkit.Core.Inspectors.asmdef</span></span> | <span data-ttu-id="1e0f7-457">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-457">Microsoft.MixedReality.Toolkit.Editor.Inspectors.asmdef</span></span> |
| <span data-ttu-id="1e0f7-458">MixedRealityToolkit ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-458">MixedRealityToolkit.Core.Inspectors.ServiceInspectors.asmdef</span></span> | <span data-ttu-id="1e0f7-459">MixedReality. ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-459">Microsoft.MixedReality.Toolkit.Editor.ServiceInspectors.asmdef</span></span> |
| <span data-ttu-id="1e0f7-460">MixedRealityToolkit. UtilitiesAsync. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-460">MixedRealityToolkit.Core.UtilitiesAsync.asmdef</span></span> | <span data-ttu-id="1e0f7-461">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-461">Microsoft.MixedReality.Toolkit.Async.asmdef</span></span> |
| <span data-ttu-id="1e0f7-462">Asmdef 的 MixedRealityToolkit。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-462">MixedRealityToolkit.Core.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="1e0f7-463">MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-463">Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="1e0f7-464">MixedRealityToolkit Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-464">MixedRealityToolkit.Utilities.Gltf.asmdef</span></span> | <span data-ttu-id="1e0f7-465">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-465">Microsoft.MixedReality.Toolkit.Gltf.asmdef</span></span> |
| <span data-ttu-id="1e0f7-466">MixedRealityToolkit，Gltf asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-466">MixedRealityToolkit.Utilities.Gltf.Importers.asmdef</span></span> | <span data-ttu-id="1e0f7-467">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-467">Microsoft.MixedReality.Toolkit.Gltf.Importers.asmdef</span></span> |

<span data-ttu-id="1e0f7-468">**MixedRealityToolkit。提供者**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-468">**MixedRealityToolkit.Providers**</span></span>

| <span data-ttu-id="1e0f7-469">RC2</span><span class="sxs-lookup"><span data-stu-id="1e0f7-469">RC2</span></span> | <span data-ttu-id="1e0f7-470">2.0.0</span><span class="sxs-lookup"><span data-stu-id="1e0f7-470">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="1e0f7-471">MixedRealityToolkit. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-471">MixedRealityToolkit.Providers.OpenVR.asmdef</span></span> | <span data-ttu-id="1e0f7-472">MixedReality. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-472">Microsoft.MixedReality.Toolkit.Providers.OpenVR.asmdef</span></span> |
| <span data-ttu-id="1e0f7-473">MixedRealityToolkit. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-473">MixedRealityToolkit.Providers.WindowsMixedReality.asmdef</span></span> | <span data-ttu-id="1e0f7-474">MixedReality. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-474">Microsoft.MixedReality.Toolkit.Providers.WindowsMixedReality.asmdef</span></span> |
| <span data-ttu-id="1e0f7-475">MixedRealityToolkit. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-475">MixedRealityToolkit.Providers.WindowsVoiceInput.asmdef</span></span> | <span data-ttu-id="1e0f7-476">MixedReality. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-476">Microsoft.MixedReality.Toolkit.Providers.WindowsVoiceInput.asmdef</span></span> |

<span data-ttu-id="1e0f7-477">**MixedRealityToolkit 服務**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-477">**MixedRealityToolkit.Services**</span></span>

| <span data-ttu-id="1e0f7-478">RC2</span><span class="sxs-lookup"><span data-stu-id="1e0f7-478">RC2</span></span> | <span data-ttu-id="1e0f7-479">2.0.0</span><span class="sxs-lookup"><span data-stu-id="1e0f7-479">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="1e0f7-480">MixedRealityToolkit. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-480">MixedRealityToolkit.Services.BoundarySystem.asmdef</span></span> | <span data-ttu-id="1e0f7-481">MixedReality. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-481">Microsoft.MixedReality.Toolkit.Services.BoundarySystem.asmdef</span></span> |
| <span data-ttu-id="1e0f7-482">MixedRealityToolkit. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-482">MixedRealityToolkit.Services.CameraSystem.asmdef</span></span> | <span data-ttu-id="1e0f7-483">MixedReality. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-483">Microsoft.MixedReality.Toolkit.Services.CameraSystem.asmdef</span></span> |
| <span data-ttu-id="1e0f7-484">MixedRealityToolkit. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-484">MixedRealityToolkit.Services.DiagnosticsSystem.asmdef</span></span> | <span data-ttu-id="1e0f7-485">MixedReality. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-485">Microsoft.MixedReality.Toolkit.Services.DiagnosticsSystem.asmdef</span></span> |
| <span data-ttu-id="1e0f7-486">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-486">MixedRealityToolkit.Services.InputSimulation.asmdef</span></span> | <span data-ttu-id="1e0f7-487">MixedReality. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-487">Microsoft.MixedReality.Toolkit.Services.InputSimulation.asmdef</span></span> |
| <span data-ttu-id="1e0f7-488">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-488">MixedRealityToolkit.Services.InputSimulation.Editor.asmdef</span></span> | <span data-ttu-id="1e0f7-489">MixedReality. InputSimulation. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-489">Microsoft.MixedReality.Toolkit.Services.InputSimulation.Editor.asmdef</span></span> |
| <span data-ttu-id="1e0f7-490">MixedRealityToolkit. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-490">MixedRealityToolkit.Services.InputSystem.asmdef</span></span> | <span data-ttu-id="1e0f7-491">MixedReality. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-491">Microsoft.MixedReality.Toolkit.Services.InputSystem.asmdef</span></span> |
| <span data-ttu-id="1e0f7-492">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-492">MixedRealityToolkit.Services.Inspectors.asmdef</span></span> | <span data-ttu-id="1e0f7-493">MixedReality. InputSystem. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-493">Microsoft.MixedReality.Toolkit.Services.InputSystem.Editor.asmdef</span></span> |
| <span data-ttu-id="1e0f7-494">MixedRealityToolkit. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-494">MixedRealityToolkit.Services.SceneSystem.asmdef</span></span> | <span data-ttu-id="1e0f7-495">MixedReality. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-495">Microsoft.MixedReality.Toolkit.Services.SceneSystem.asmdef</span></span> |
| <span data-ttu-id="1e0f7-496">MixedRealityToolkit. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-496">MixedRealityToolkit.Services.SpatialAwarenessSystem.asmdef</span></span> | <span data-ttu-id="1e0f7-497">MixedReality. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-497">Microsoft.MixedReality.Toolkit.Services.SpatialAwarenessSystem.asmdef</span></span> |
| <span data-ttu-id="1e0f7-498">MixedRealityToolkit. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-498">MixedRealityToolkit.Services.TeleportSystem.asmdef</span></span> | <span data-ttu-id="1e0f7-499">MixedReality. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-499">Microsoft.MixedReality.Toolkit.Services.TeleportSystem.asmdef</span></span> |

<span data-ttu-id="1e0f7-500">**MixedRealityToolkit SDK**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-500">**MixedRealityToolkit.SDK**</span></span>

| <span data-ttu-id="1e0f7-501">RC2</span><span class="sxs-lookup"><span data-stu-id="1e0f7-501">RC2</span></span> | <span data-ttu-id="1e0f7-502">2.0.0</span><span class="sxs-lookup"><span data-stu-id="1e0f7-502">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="1e0f7-503">MixedRealityToolkit. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-503">MixedRealityToolkit.SDK.asmdef</span></span> | <span data-ttu-id="1e0f7-504">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-504">Microsoft.MixedReality.Toolkit.SDK.asmdef</span></span> |
| <span data-ttu-id="1e0f7-505">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-505">MixedRealityToolkit.SDK.Inspectors.asmdef</span></span> | <span data-ttu-id="1e0f7-506">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-506">Microsoft.MixedReality.Toolkit.SDK.Inspectors.asmdef</span></span> |

<span data-ttu-id="1e0f7-507">**MixedRealityToolkit 範例**</span><span class="sxs-lookup"><span data-stu-id="1e0f7-507">**MixedRealityToolkit.Examples**</span></span>

| <span data-ttu-id="1e0f7-508">RC2</span><span class="sxs-lookup"><span data-stu-id="1e0f7-508">RC2</span></span> | <span data-ttu-id="1e0f7-509">2.0.0</span><span class="sxs-lookup"><span data-stu-id="1e0f7-509">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="1e0f7-510">MixedRealityToolkit。例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-510">MixedRealityToolkit.Examples.asmdef</span></span> | <span data-ttu-id="1e0f7-511">MixedReality，例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-511">Microsoft.MixedReality.Toolkit.Examples.asmdef</span></span> |
| <span data-ttu-id="1e0f7-512">MixedRealityToolkit. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-512">MixedRealityToolkit.Examples.Demos.Gltf.asmdef</span></span> | <span data-ttu-id="1e0f7-513">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-513">Microsoft.MixedReality.Toolkit.Demos.Gltf.asmdef</span></span> |
| <span data-ttu-id="1e0f7-514">MixedRealityToolkit，例如 StandardShader asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-514">MixedRealityToolkit.Examples.Demos.StandardShader.Inspectors.asmdef</span></span> | <span data-ttu-id="1e0f7-515">MixedReality. StandardShader. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-515">Microsoft.MixedReality.Toolkit.Demos.StandardShader.Inspectors.asmdef</span></span> |
| <span data-ttu-id="1e0f7-516">MixedRealityToolkit，InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-516">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.asmdef</span></span> | <span data-ttu-id="1e0f7-517">MixedReality. InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-517">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.asmdef</span></span> |
| <span data-ttu-id="1e0f7-518">MixedRealityToolkit，例如 InspectorFields asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-518">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.Inspectors.asmdef</span></span> | <span data-ttu-id="1e0f7-519">MixedReality. InspectorFields. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-519">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.Inspectors.asmdef</span></span> |
| <span data-ttu-id="1e0f7-520">MixedRealityToolkit，例如 Interactables. asmdef</span><span class="sxs-lookup"><span data-stu-id="1e0f7-520">MixedRealityToolkit.Examples.Demos.UX.Interactables.asmdef</span></span> | <span data-ttu-id="1e0f7-521">MixedReality. Interactables. asmdef。</span><span class="sxs-lookup"><span data-stu-id="1e0f7-521">Microsoft.MixedReality.Toolkit.Demos.UX.Interactables.asmdef</span></span> |
