...
不是 "RuntimeSwapparoo"，您可以有一組適用于特定平臺的任意設定檔 (例如，一個適用于 HoloLens 1、一個用於 VR、一個用於 HoloLens 2 等) 。 您可以使用各種其他指標，或是攝影機是否為不透明/透明的 [) ，以](https://docs.unity3d.com/ScriptReference/SystemInfo.html)找出要載入的設定檔。
