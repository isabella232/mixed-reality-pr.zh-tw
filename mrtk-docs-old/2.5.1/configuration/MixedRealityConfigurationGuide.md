...
MRTK 會提供此選用服務，協助您管理複雜的加法場景載入/卸載。 若要判斷場景系統是否適合您的專案，請閱讀「 [場景系統入門指南」。](../features/scene-system/SceneSystemGettingStarted.md)
...
除了 "RuntimeSwapparoo" 之外，還可以有一組適用于特定平臺的任意設定檔 (例如，一個適用于 HoloLens 1、一個用於 VR、一個用於 HoloLens 2，依此類推) 。 您可以使用各種其他指標，或是攝影機是否為不透明/透明的 [) ，以](https://docs.unity3d.com/ScriptReference/SystemInfo.html)找出要載入的設定檔。
