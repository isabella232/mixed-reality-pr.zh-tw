...
### <a name="unity-asset-unitypackage-files"></a><span data-ttu-id="ce78a-116">Unity 資產 (. unitypackage) 檔案</span><span class="sxs-lookup"><span data-stu-id="ce78a-116">Unity asset (.unitypackage) files</span></span>
...
1. <span data-ttu-id="ce78a-140">關閉 Unity 並刪除連結 **庫** 資料夾 (先閱讀以下的附注！ ) 。</span><span class="sxs-lookup"><span data-stu-id="ce78a-140">Close Unity and delete the **Library** folder (read the note below first!).</span></span> <span data-ttu-id="ce78a-141">必須執行此步驟，才能強制 Unity 重新整理其資產資料庫，並協調現有的自訂設定檔。</span><span class="sxs-lookup"><span data-stu-id="ce78a-141">This step is necessary to force Unity to refresh its asset database and reconcile existing custom profiles.</span></span>
...
<span data-ttu-id="ce78a-158">[資料夾重新命名](#folder-renames-in-240) 
...
### <a name="eye-gaze-pointer-behavior-in-240"></a><span data-ttu-id="ce78a-186">2.4.0 中的眼睛指標行為</span><span class="sxs-lookup"><span data-stu-id="ce78a-186">Eye gaze pointer behavior in 2.4.0</span></span>
...
<span data-ttu-id="ce78a-200">現在就完成了 .。。</span><span class="sxs-lookup"><span data-stu-id="ce78a-200">Do this now...</span></span>
...
<span data-ttu-id="ce78a-239">雖然現有的程式碼在版本2.3.0 中仍可繼續運作，但建議您更新至新的類別。</span><span class="sxs-lookup"><span data-stu-id="ce78a-239">While existing code will continue to function in version 2.3.0, it is recommended to update to the new class.</span></span>
...
<span data-ttu-id="ce78a-260">應用程式程式碼可能會因為這種變更而需要更新。</span><span class="sxs-lookup"><span data-stu-id="ce78a-260">Application code may require updating due to this change.</span></span>
...
- [<span data-ttu-id="ce78a-276">API 變更</span><span class="sxs-lookup"><span data-stu-id="ce78a-276">API changes</span></span>](#api-changes-in-210)
...
<span data-ttu-id="ce78a-294">**互動**</span><span class="sxs-lookup"><span data-stu-id="ce78a-294">**Interactable**</span></span>
...
<span data-ttu-id="ce78a-324">如需這些變更的詳細資訊，請參閱「 [輸入模擬服務](../features/input-simulation/input-simulation-service.md) 」一文。</span><span class="sxs-lookup"><span data-stu-id="ce78a-324">For more information on these changes, please see the [input simulation service](../features/input-simulation/input-simulation-service.md) article.</span></span>
...
<span data-ttu-id="ce78a-341">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="ce78a-341">**MixedRealityToolkit**</span></span>
...
<span data-ttu-id="ce78a-351">使用新的 CoreSystem 類別可確保您的應用程式程式碼在您將應用程式變更為使用不同的服務註冊機構時，不需要更新 (例如：其中一個實驗性服務管理員) 。</span><span class="sxs-lookup"><span data-stu-id="ce78a-351">Using the new CoreSystem class will ensure that your application code will not need updating if you change the application to use a different service registrar (ex: one of the experimental service managers).</span></span>
...
<span data-ttu-id="ce78a-370">新 `RegisterHandler` 的 API 和 `UnregisterHandler` ：</span><span class="sxs-lookup"><span data-stu-id="ce78a-370">New API `RegisterHandler` and `UnregisterHandler`:</span></span>
...
<span data-ttu-id="ce78a-401">**_SurfaceMagnetism_**</span><span class="sxs-lookup"><span data-stu-id="ce78a-401">**_SurfaceMagnetism_**</span></span>
...
- <span data-ttu-id="ce78a-421">BaseNearInteractionTouchable 已重新命名為 PokeThreshold，因此它會清楚指出 PokeThreshold 是 DebounceThreshold 的對應項。</span><span class="sxs-lookup"><span data-stu-id="ce78a-421">BaseNearInteractionTouchable.DistBack has been renamed to PokeThreshold This makes it clear that PokeThreshold is the counterpart to DebounceThreshold.</span></span> <span data-ttu-id="ce78a-422">當超過 PokeThreshold 時，即會啟動可觸式，並在超過 DebounceThreshold 時釋放。</span><span class="sxs-lookup"><span data-stu-id="ce78a-422">A touchable is activated when the PokeThreshold is crossed, and released when DebounceThreshold is crossed.</span></span>
...
<span data-ttu-id="ce78a-433">`InteractableHighlight` 類別已被取代。</span><span class="sxs-lookup"><span data-stu-id="ce78a-433">The `InteractableHighlight` class has been deprecated.</span></span> <span data-ttu-id="ce78a-434">`InteractableOnFocus` `FocusInteractableStates` 應改為使用類別和資產。</span><span class="sxs-lookup"><span data-stu-id="ce78a-434">The `InteractableOnFocus` class and `FocusInteractableStates` asset should be used instead.</span></span> <span data-ttu-id="ce78a-435">若要為建立新的 `Theme` 資產 `InteractableOnFocus` ，請在 [專案] 視窗中按一下滑鼠右鍵，然後選取 [*建立*  >  *混合現實工具* 組  >  *互動*  >  *主題*]。</span><span class="sxs-lookup"><span data-stu-id="ce78a-435">To create a new `Theme` asset for the `InteractableOnFocus`, right click in the project window and select *Create* > *Mixed Reality Toolkit* > *Interactable* > *Theme*.</span></span>

<span data-ttu-id="ce78a-436">**HandInteractionPanZoom**</span><span class="sxs-lookup"><span data-stu-id="ce78a-436">**HandInteractionPanZoom**</span></span>

<span data-ttu-id="ce78a-437">`HandInteractionPanZoom` 已移至 UI 命名空間，因為它不是輸入元件。</span><span class="sxs-lookup"><span data-stu-id="ce78a-437">`HandInteractionPanZoom` has been moved to the UI namespace as it was not an input component.</span></span> <span data-ttu-id="ce78a-438">`HandPanEventData` 也已移至此命名空間，並簡化為與其他 UI 事件資料對應。</span><span class="sxs-lookup"><span data-stu-id="ce78a-438">`HandPanEventData` has also been moved into this namespace, and simplified to correspond with other UI event data.</span></span>

### <a name="assembly-name-changes-in-200"></a><span data-ttu-id="ce78a-439">2.0.0 中的元件名稱變更</span><span class="sxs-lookup"><span data-stu-id="ce78a-439">Assembly name changes in 2.0.0</span></span>

<span data-ttu-id="ce78a-440">在2.0.0 版本中，所有的官方混合現實工具組元件名稱及其相關聯的元件定義 (. asmdef) 檔案都已更新為符合下列模式。</span><span class="sxs-lookup"><span data-stu-id="ce78a-440">In The 2.0.0 release, all of the official Mixed Reality Toolkit assembly names and their associated assembly definition (.asmdef) files have been updated to fit the following pattern.</span></span>

```c#
Microsoft.MixedReality.Toolkit[.<name>]
```

<span data-ttu-id="ce78a-441">在某些情況下，已合併多個元件以建立更好的 unity 內容。</span><span class="sxs-lookup"><span data-stu-id="ce78a-441">In some instances, multiple assemblies have been merged to create better unity of their contents.</span></span> <span data-ttu-id="ce78a-442">如果您的專案使用自訂的 asmdef 檔案，則可能需要更新。</span><span class="sxs-lookup"><span data-stu-id="ce78a-442">If your project uses custom .asmdef files, they may require updating.</span></span>

<span data-ttu-id="ce78a-443">下表說明 RC2 asmdef 檔案名如何對應至2.0.0 版本。</span><span class="sxs-lookup"><span data-stu-id="ce78a-443">The following tables describe how the RC2 .asmdef file names map to the 2.0.0 release.</span></span> <span data-ttu-id="ce78a-444">所有元件名稱都符合 asmdef 檔案名。</span><span class="sxs-lookup"><span data-stu-id="ce78a-444">All assembly names match the .asmdef file name.</span></span>

<span data-ttu-id="ce78a-445">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="ce78a-445">**MixedRealityToolkit**</span></span>

| <span data-ttu-id="ce78a-446">RC2</span><span class="sxs-lookup"><span data-stu-id="ce78a-446">RC2</span></span> | <span data-ttu-id="ce78a-447">2.0.0</span><span class="sxs-lookup"><span data-stu-id="ce78a-447">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="ce78a-448">MixedRealityToolkit.asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-448">MixedRealityToolkit.asmdef</span></span> | <span data-ttu-id="ce78a-449">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-449">Microsoft.MixedReality.Toolkit.asmdef</span></span> |
| <span data-ttu-id="ce78a-450">MixedRealityToolkit. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-450">MixedRealityToolkit.Core.BuildAndDeploy.asmdef</span></span> | <span data-ttu-id="ce78a-451">MixedReality. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-451">Microsoft.MixedReality.Toolkit.Editor.BuildAndDeploy.asmdef</span></span> |
| <span data-ttu-id="ce78a-452">MixedRealityToolkit，asmdef 的</span><span class="sxs-lookup"><span data-stu-id="ce78a-452">MixedRealityToolkit.Core.Definitions.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="ce78a-453">已移除，請使用 MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-453">Removed, use Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="ce78a-454">MixedRealityToolkit. EditorClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-454">MixedRealityToolkit.Core.Extensions.EditorClassExtensions.asmdef</span></span> | <span data-ttu-id="ce78a-455">MixedReality. ClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-455">Microsoft.MixedReality.Toolkit.Editor.ClassExtensions.asmdef</span></span>
| <span data-ttu-id="ce78a-456">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-456">MixedRealityToolkit.Core.Inspectors.asmdef</span></span> | <span data-ttu-id="ce78a-457">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-457">Microsoft.MixedReality.Toolkit.Editor.Inspectors.asmdef</span></span> |
| <span data-ttu-id="ce78a-458">MixedRealityToolkit ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-458">MixedRealityToolkit.Core.Inspectors.ServiceInspectors.asmdef</span></span> | <span data-ttu-id="ce78a-459">MixedReality. ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-459">Microsoft.MixedReality.Toolkit.Editor.ServiceInspectors.asmdef</span></span> |
| <span data-ttu-id="ce78a-460">MixedRealityToolkit. UtilitiesAsync. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-460">MixedRealityToolkit.Core.UtilitiesAsync.asmdef</span></span> | <span data-ttu-id="ce78a-461">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-461">Microsoft.MixedReality.Toolkit.Async.asmdef</span></span> |
| <span data-ttu-id="ce78a-462">Asmdef 的 MixedRealityToolkit。</span><span class="sxs-lookup"><span data-stu-id="ce78a-462">MixedRealityToolkit.Core.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="ce78a-463">MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-463">Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="ce78a-464">MixedRealityToolkit Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-464">MixedRealityToolkit.Utilities.Gltf.asmdef</span></span> | <span data-ttu-id="ce78a-465">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-465">Microsoft.MixedReality.Toolkit.Gltf.asmdef</span></span> |
| <span data-ttu-id="ce78a-466">MixedRealityToolkit，Gltf asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-466">MixedRealityToolkit.Utilities.Gltf.Importers.asmdef</span></span> | <span data-ttu-id="ce78a-467">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-467">Microsoft.MixedReality.Toolkit.Gltf.Importers.asmdef</span></span> |

<span data-ttu-id="ce78a-468">**MixedRealityToolkit。提供者**</span><span class="sxs-lookup"><span data-stu-id="ce78a-468">**MixedRealityToolkit.Providers**</span></span>

| <span data-ttu-id="ce78a-469">RC2</span><span class="sxs-lookup"><span data-stu-id="ce78a-469">RC2</span></span> | <span data-ttu-id="ce78a-470">2.0.0</span><span class="sxs-lookup"><span data-stu-id="ce78a-470">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="ce78a-471">MixedRealityToolkit. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-471">MixedRealityToolkit.Providers.OpenVR.asmdef</span></span> | <span data-ttu-id="ce78a-472">MixedReality. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-472">Microsoft.MixedReality.Toolkit.Providers.OpenVR.asmdef</span></span> |
| <span data-ttu-id="ce78a-473">MixedRealityToolkit. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-473">MixedRealityToolkit.Providers.WindowsMixedReality.asmdef</span></span> | <span data-ttu-id="ce78a-474">MixedReality. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-474">Microsoft.MixedReality.Toolkit.Providers.WindowsMixedReality.asmdef</span></span> |
| <span data-ttu-id="ce78a-475">MixedRealityToolkit. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-475">MixedRealityToolkit.Providers.WindowsVoiceInput.asmdef</span></span> | <span data-ttu-id="ce78a-476">MixedReality. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-476">Microsoft.MixedReality.Toolkit.Providers.WindowsVoiceInput.asmdef</span></span> |

<span data-ttu-id="ce78a-477">**MixedRealityToolkit 服務**</span><span class="sxs-lookup"><span data-stu-id="ce78a-477">**MixedRealityToolkit.Services**</span></span>

| <span data-ttu-id="ce78a-478">RC2</span><span class="sxs-lookup"><span data-stu-id="ce78a-478">RC2</span></span> | <span data-ttu-id="ce78a-479">2.0.0</span><span class="sxs-lookup"><span data-stu-id="ce78a-479">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="ce78a-480">MixedRealityToolkit. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-480">MixedRealityToolkit.Services.BoundarySystem.asmdef</span></span> | <span data-ttu-id="ce78a-481">MixedReality. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-481">Microsoft.MixedReality.Toolkit.Services.BoundarySystem.asmdef</span></span> |
| <span data-ttu-id="ce78a-482">MixedRealityToolkit. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-482">MixedRealityToolkit.Services.CameraSystem.asmdef</span></span> | <span data-ttu-id="ce78a-483">MixedReality. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-483">Microsoft.MixedReality.Toolkit.Services.CameraSystem.asmdef</span></span> |
| <span data-ttu-id="ce78a-484">MixedRealityToolkit. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-484">MixedRealityToolkit.Services.DiagnosticsSystem.asmdef</span></span> | <span data-ttu-id="ce78a-485">MixedReality. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-485">Microsoft.MixedReality.Toolkit.Services.DiagnosticsSystem.asmdef</span></span> |
| <span data-ttu-id="ce78a-486">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-486">MixedRealityToolkit.Services.InputSimulation.asmdef</span></span> | <span data-ttu-id="ce78a-487">MixedReality. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-487">Microsoft.MixedReality.Toolkit.Services.InputSimulation.asmdef</span></span> |
| <span data-ttu-id="ce78a-488">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-488">MixedRealityToolkit.Services.InputSimulation.Editor.asmdef</span></span> | <span data-ttu-id="ce78a-489">MixedReality. InputSimulation. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-489">Microsoft.MixedReality.Toolkit.Services.InputSimulation.Editor.asmdef</span></span> |
| <span data-ttu-id="ce78a-490">MixedRealityToolkit. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-490">MixedRealityToolkit.Services.InputSystem.asmdef</span></span> | <span data-ttu-id="ce78a-491">MixedReality. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-491">Microsoft.MixedReality.Toolkit.Services.InputSystem.asmdef</span></span> |
| <span data-ttu-id="ce78a-492">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-492">MixedRealityToolkit.Services.Inspectors.asmdef</span></span> | <span data-ttu-id="ce78a-493">MixedReality. InputSystem. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-493">Microsoft.MixedReality.Toolkit.Services.InputSystem.Editor.asmdef</span></span> |
| <span data-ttu-id="ce78a-494">MixedRealityToolkit. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-494">MixedRealityToolkit.Services.SceneSystem.asmdef</span></span> | <span data-ttu-id="ce78a-495">MixedReality. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-495">Microsoft.MixedReality.Toolkit.Services.SceneSystem.asmdef</span></span> |
| <span data-ttu-id="ce78a-496">MixedRealityToolkit. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-496">MixedRealityToolkit.Services.SpatialAwarenessSystem.asmdef</span></span> | <span data-ttu-id="ce78a-497">MixedReality. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-497">Microsoft.MixedReality.Toolkit.Services.SpatialAwarenessSystem.asmdef</span></span> |
| <span data-ttu-id="ce78a-498">MixedRealityToolkit. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-498">MixedRealityToolkit.Services.TeleportSystem.asmdef</span></span> | <span data-ttu-id="ce78a-499">MixedReality. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-499">Microsoft.MixedReality.Toolkit.Services.TeleportSystem.asmdef</span></span> |

<span data-ttu-id="ce78a-500">**MixedRealityToolkit SDK**</span><span class="sxs-lookup"><span data-stu-id="ce78a-500">**MixedRealityToolkit.SDK**</span></span>

| <span data-ttu-id="ce78a-501">RC2</span><span class="sxs-lookup"><span data-stu-id="ce78a-501">RC2</span></span> | <span data-ttu-id="ce78a-502">2.0.0</span><span class="sxs-lookup"><span data-stu-id="ce78a-502">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="ce78a-503">MixedRealityToolkit. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-503">MixedRealityToolkit.SDK.asmdef</span></span> | <span data-ttu-id="ce78a-504">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-504">Microsoft.MixedReality.Toolkit.SDK.asmdef</span></span> |
| <span data-ttu-id="ce78a-505">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-505">MixedRealityToolkit.SDK.Inspectors.asmdef</span></span> | <span data-ttu-id="ce78a-506">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-506">Microsoft.MixedReality.Toolkit.SDK.Inspectors.asmdef</span></span> |

<span data-ttu-id="ce78a-507">**MixedRealityToolkit 範例**</span><span class="sxs-lookup"><span data-stu-id="ce78a-507">**MixedRealityToolkit.Examples**</span></span>

| <span data-ttu-id="ce78a-508">RC2</span><span class="sxs-lookup"><span data-stu-id="ce78a-508">RC2</span></span> | <span data-ttu-id="ce78a-509">2.0.0</span><span class="sxs-lookup"><span data-stu-id="ce78a-509">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="ce78a-510">MixedRealityToolkit。例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-510">MixedRealityToolkit.Examples.asmdef</span></span> | <span data-ttu-id="ce78a-511">MixedReality，例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-511">Microsoft.MixedReality.Toolkit.Examples.asmdef</span></span> |
| <span data-ttu-id="ce78a-512">MixedRealityToolkit. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-512">MixedRealityToolkit.Examples.Demos.Gltf.asmdef</span></span> | <span data-ttu-id="ce78a-513">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-513">Microsoft.MixedReality.Toolkit.Demos.Gltf.asmdef</span></span> |
| <span data-ttu-id="ce78a-514">MixedRealityToolkit，例如 StandardShader asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-514">MixedRealityToolkit.Examples.Demos.StandardShader.Inspectors.asmdef</span></span> | <span data-ttu-id="ce78a-515">MixedReality. StandardShader. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-515">Microsoft.MixedReality.Toolkit.Demos.StandardShader.Inspectors.asmdef</span></span> |
| <span data-ttu-id="ce78a-516">MixedRealityToolkit，InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-516">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.asmdef</span></span> | <span data-ttu-id="ce78a-517">MixedReality. InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-517">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.asmdef</span></span> |
| <span data-ttu-id="ce78a-518">MixedRealityToolkit，例如 InspectorFields asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-518">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.Inspectors.asmdef</span></span> | <span data-ttu-id="ce78a-519">MixedReality. InspectorFields. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-519">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.Inspectors.asmdef</span></span> |
| <span data-ttu-id="ce78a-520">MixedRealityToolkit，例如 Interactables. asmdef</span><span class="sxs-lookup"><span data-stu-id="ce78a-520">MixedRealityToolkit.Examples.Demos.UX.Interactables.asmdef</span></span> | <span data-ttu-id="ce78a-521">MixedReality. Interactables. asmdef。</span><span class="sxs-lookup"><span data-stu-id="ce78a-521">Microsoft.MixedReality.Toolkit.Demos.UX.Interactables.asmdef</span></span> |
