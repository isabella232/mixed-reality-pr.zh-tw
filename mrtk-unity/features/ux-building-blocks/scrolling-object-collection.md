...
> [!IMPORTANT]
> 任何滾動的內容物件材質都必須使用 [MRTK 標準著色器](../rendering/MRTK-standard-shader.md) ，才能讓可見區域的剪切效果正常運作。

> [!NOTE]
> 如果已啟用滾動內容遮罩，則滾動物件集合會將 [材質實例](../rendering/material-instance.md) 元件加入至已附加轉譯器的任何內容物件。 此元件可用來管理實例材質存留期，並改善記憶體效能。
...
| 進階設定           | Description                                                                                                                                                                |
| :-------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 遮罩編輯模式              | 用來定義裁剪方塊遮罩界限的編輯模式。 ' Auto ' 會自動使用分頁值。 「手動」可直接操作裁剪 box 物件。 |
| 碰撞編輯模式          | 編輯模式，用於定義捲軸互動碰撞條界限。 ' Auto ' 會自動使用分頁值。 「手動」可直接操作碰撞。     |
| 可滾動                  | 啟用/停用近乎/遠的互動的滾動。                                                                                                                      |
...
| 事件              | 描述                                                                                                                   |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 按一下時            | 當捲軸背景碰撞或其任何互動式內容收到點擊時，即會觸發。                             |
| 開始觸控時    | 當滾動背景碰撞項或其任何互動式內容收到接近互動觸控時觸發。            |
| 在觸控結束時      | 當接近互動指標跨越發行閾值平面時，會觸發作用中觸控互動。 |
| 開始使用時 | 當滾動容器以互動、速度衰減或分頁開始移動時觸發。                            |
| 動力已結束   | 當捲軸容器停止互動、速度衰減或分頁時，就會觸發。                             |
...
* [裁剪基本](../rendering/clipping-primitive.md)
* [材質實例](../rendering/material-instance.md)
* [標準著色器](../rendering/MRTK-standard-shader.md)
