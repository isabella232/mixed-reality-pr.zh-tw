...
<span data-ttu-id="a02dc-116">如果上述的檔案和資料夾不存在，您就會使用較新版本的 MRTK。</span><span class="sxs-lookup"><span data-stu-id="a02dc-116">If the file and folder above doesn't exist, you're on a newer version of the MRTK.</span></span> <span data-ttu-id="a02dc-117">在此情況下，請嘗試下列動作：</span><span class="sxs-lookup"><span data-stu-id="a02dc-117">In that case, try the following:</span></span>
...
<span data-ttu-id="a02dc-173">從 Unity 資產檔案切換至混合現實功能工具套件帶來許多優點：</span><span class="sxs-lookup"><span data-stu-id="a02dc-173">Switching from Unity asset files to Mixed Reality Feature Tool packages brings a number of benefits:</span></span>
...
<span data-ttu-id="a02dc-195">執行上述步驟之後，請執行 [混合現實功能工具](#mixed-reality-feature-tool) ，並匯入所需的混合現實工具組版本。</span><span class="sxs-lookup"><span data-stu-id="a02dc-195">Once the previous steps have been performed, run the [Mixed Reality Feature Tool](#mixed-reality-feature-tool) and import the desired version of the Mixed Reality Toolkit.</span></span>
...
<span data-ttu-id="a02dc-221">此版本的 MRTK 會修改眼睛設定所需的步驟。</span><span class="sxs-lookup"><span data-stu-id="a02dc-221">This version of MRTK modifies the steps required for eye gaze setup.</span></span> <span data-ttu-id="a02dc-222">您可以在輸入指標設定檔的注視設定中找到 [ _IsEyeTrackingEnabled_ ] 核取方塊。</span><span class="sxs-lookup"><span data-stu-id="a02dc-222">The _'IsEyeTrackingEnabled'_ checkbox can be found in the gaze settings of the input pointer profile.</span></span> <span data-ttu-id="a02dc-223">核取此方塊將會啟用以眼睛為基礎的眼睛，而不是預設的標頭。</span><span class="sxs-lookup"><span data-stu-id="a02dc-223">Checking this box will enable eye based gaze, rather then the default head based gaze.</span></span>
...
<span data-ttu-id="a02dc-237">`UseEyeTracking`從執行的屬性 `GazeProvider` 已重新 `IMixedRealityEyeGazeProvider` 命名為 `IsEyeTrackingEnabled` 。</span><span class="sxs-lookup"><span data-stu-id="a02dc-237">The `UseEyeTracking` property from `GazeProvider` implementation of `IMixedRealityEyeGazeProvider` was renamed to `IsEyeTrackingEnabled`.</span></span>
...
### <a name="custom-button-icon-migration"></a><span data-ttu-id="a02dc-253">自訂按鈕圖示遷移</span><span class="sxs-lookup"><span data-stu-id="a02dc-253">Custom Button Icon Migration</span></span>
...
<span data-ttu-id="a02dc-270">已 ([#7012](https://github.com/microsoft/MixedRealityToolkit-Unity/pull/7012)) 移除 public ControllerPoseSynchronizer. Handedness 屬性的 setter。</span><span class="sxs-lookup"><span data-stu-id="a02dc-270">The public ControllerPoseSynchronizer.Handedness property's setter has been removed ([#7012](https://github.com/microsoft/MixedRealityToolkit-Unity/pull/7012)).</span></span>
...
<span data-ttu-id="a02dc-284">不需要進行任何變更，即可充分利用此修正。</span><span class="sxs-lookup"><span data-stu-id="a02dc-284">No changes are required to take advantage of this fix.</span></span>
...
- [<span data-ttu-id="a02dc-304">API 變更</span><span class="sxs-lookup"><span data-stu-id="a02dc-304">API changes</span></span>](#api-changes-in-220)
...
<span data-ttu-id="a02dc-321">**ColliderNearInteractionTouchable**</span><span class="sxs-lookup"><span data-stu-id="a02dc-321">**ColliderNearInteractionTouchable**</span></span>
...
1. <span data-ttu-id="a02dc-348">設定檔中的所有 KeyCode 和滑鼠按鍵系結都已取代為泛型 `KeyBinding` 結構，此結構會儲存系結類型 (按鍵或滑鼠) 以及實際的系結程式碼， (KeyCode 或滑鼠按鍵編號分別) 。</span><span class="sxs-lookup"><span data-stu-id="a02dc-348">All KeyCode and mouse button bindings in the profile have been replaced with a generic `KeyBinding` struct, which stores the type of binding (key or mouse) as well as the actual binding code (KeyCode or mouse button number respectively).</span></span> <span data-ttu-id="a02dc-349">此結構有自己的偵測器，可允許統一顯示並提供「自動系結」工具，只要按下個別的按鍵即可快速設定按鍵系結，而不需要從大型的下拉式清單中選取。</span><span class="sxs-lookup"><span data-stu-id="a02dc-349">The struct has its own inspector, which allows unified display and offers an "auto-bind" tool to quickly set key bindings by pressing the respective key instead of selecting from a huge dropdown list.</span></span>
    ...
    - <span data-ttu-id="a02dc-350">FastControlKey</span><span class="sxs-lookup"><span data-stu-id="a02dc-350">FastControlKey</span></span>
...
<span data-ttu-id="a02dc-369">某些軸和輸入類型已在2.1.0 中更新，尤其是在 OpenVR 平臺上。</span><span class="sxs-lookup"><span data-stu-id="a02dc-369">Some axes and input types have been updated in 2.1.0, especially around the OpenVR platform.</span></span> <span data-ttu-id="a02dc-370">升級時，請務必選取 [ **MixedRealityToolkit-> 公用程式-> 更新 > 控制器對應設定檔** ]。</span><span class="sxs-lookup"><span data-stu-id="a02dc-370">Please be sure to select **MixedRealityToolkit -> Utilities -> Update -> Controller Mapping Profiles** when upgrading.</span></span> <span data-ttu-id="a02dc-371">這會將任何自訂控制器對應設定檔更新為更新的座標軸和資料，同時讓您的自訂指派輸入動作保持不變。</span><span class="sxs-lookup"><span data-stu-id="a02dc-371">This will update any custom Controller Mapping Profiles with the updated axes and data, while leaving your custom-assigned input actions intact.</span></span>
...
<span data-ttu-id="a02dc-412">**_如何遷移_**</span><span class="sxs-lookup"><span data-stu-id="a02dc-412">**_How to migrate_**</span></span>
...
<span data-ttu-id="a02dc-424">下列方法 (s) 已重新命名，以更清楚地描述其使用方式。</span><span class="sxs-lookup"><span data-stu-id="a02dc-424">The following method(s) have been renamed to better describe their usage.</span></span>
...
- <span data-ttu-id="a02dc-441">`MaxDistance` 公用屬性已被取代，並已重新命名為 `MaxRaycastDistance`</span><span class="sxs-lookup"><span data-stu-id="a02dc-441">`MaxDistance` public property deprecated and has been renamed to `MaxRaycastDistance`</span></span>
- <span data-ttu-id="a02dc-442">`CloseDistance` 公用屬性已被取代，並已重新命名為 `ClosestDistance`</span><span class="sxs-lookup"><span data-stu-id="a02dc-442">`CloseDistance` public property deprecated and has been renamed to `ClosestDistance`</span></span>
- <span data-ttu-id="a02dc-443">的預設值 `RaycastDirectionMode` 現在會 `TrackedTargetForward` 以追蹤的目標轉換方向向前 raycasts</span><span class="sxs-lookup"><span data-stu-id="a02dc-443">Default value for `RaycastDirectionMode` is now `TrackedTargetForward` which raycasts in the direction of the tracked target transform forward</span></span>
- <span data-ttu-id="a02dc-444">`OrientationMode`列舉值 `Vertical` 和 `Full` 都已 `TrackedTarget` 分別重新命名為 `SurfaceNormal`</span><span class="sxs-lookup"><span data-stu-id="a02dc-444">`OrientationMode` enum values, `Vertical` and `Full`, have been renamed to `TrackedTarget` and `SurfaceNormal` respectively</span></span>
- <span data-ttu-id="a02dc-445">`KeepOrientationVertical` 已新增公用屬性，以控制相關聯 GameObject 的方向是否維持垂直</span><span class="sxs-lookup"><span data-stu-id="a02dc-445">`KeepOrientationVertical` public property has been added to control whether orientation of associated GameObject remains vertical</span></span>

<span data-ttu-id="a02dc-446">**按鈕**</span><span class="sxs-lookup"><span data-stu-id="a02dc-446">**Buttons**</span></span>

- <span data-ttu-id="a02dc-447">[`PressableButton`](xref:Microsoft.MixedReality.Toolkit.UI.PressableButton) 現在 `DistanceSpaceMode` 將屬性設為 `Local` 預設值。</span><span class="sxs-lookup"><span data-stu-id="a02dc-447">[`PressableButton`](xref:Microsoft.MixedReality.Toolkit.UI.PressableButton) now has `DistanceSpaceMode` property set to `Local` as default.</span></span> <span data-ttu-id="a02dc-448">這可讓按鈕進行調整，同時仍 pressable</span><span class="sxs-lookup"><span data-stu-id="a02dc-448">This allows buttons to be scaled while still be pressable</span></span>

<span data-ttu-id="a02dc-449">**剪出球體**</span><span class="sxs-lookup"><span data-stu-id="a02dc-449">**Clipping Sphere**</span></span>

<span data-ttu-id="a02dc-450">ClippingSphere 介面已變更，以反映在 ClippingBox 和 ClippingPlane 中找到的 Api。</span><span class="sxs-lookup"><span data-stu-id="a02dc-450">The ClippingSphere interface has changed to mirror the APIs found in the ClippingBox and ClippingPlane.</span></span>

<span data-ttu-id="a02dc-451">現在會根據轉換比例，以隱含的方式來計算 ClippingSphere 的 Radius 屬性。</span><span class="sxs-lookup"><span data-stu-id="a02dc-451">The ClippingSphere's Radius property is now implicitly calculated based on the transform scale.</span></span> <span data-ttu-id="a02dc-452">開發人員必須先在偵測器中指定 ClippingSphere 的半徑。</span><span class="sxs-lookup"><span data-stu-id="a02dc-452">Before developers would have to specify the radius of the ClippingSphere in the inspector.</span></span> <span data-ttu-id="a02dc-453">如果您想要變更半徑，只要以平常的方式更新轉換的轉換規模。</span><span class="sxs-lookup"><span data-stu-id="a02dc-453">If you want to change the radius, just update the transform scale of the transform as you normally would.</span></span>

<span data-ttu-id="a02dc-454">**NearInteractionTouchable 和 PokePointer**</span><span class="sxs-lookup"><span data-stu-id="a02dc-454">**NearInteractionTouchable and PokePointer**</span></span>

- <span data-ttu-id="a02dc-455">NearInteractionTouchable 不會處理 Unity UI 畫布的任何時間。</span><span class="sxs-lookup"><span data-stu-id="a02dc-455">NearInteractionTouchable does not handle Unity UI canvas touching any longer.</span></span> <span data-ttu-id="a02dc-456">NearInteractionTouchableUnityUI 類別必須立即用於 Unity UI touchables。</span><span class="sxs-lookup"><span data-stu-id="a02dc-456">The NearInteractionTouchableUnityUI class must be used for Unity UI touchables now.</span></span>
- <span data-ttu-id="a02dc-457">ColliderNearInteractionTouchable 是以 colliders 為基礎之 touchables 的新基類，也就是 NearInteractionTouchableUnityUI 以外的每個可觸式。</span><span class="sxs-lookup"><span data-stu-id="a02dc-457">ColliderNearInteractionTouchable is the new base class for touchables based on colliders, i.e. every touchable except NearInteractionTouchableUnityUI.</span></span>
- <span data-ttu-id="a02dc-458">BaseNearInteractionTouchable. DistFront 已移動並重新命名為 PokePointer。 TouchableDistance 這是 PokePointer 可以與 touchables 互動的距離。</span><span class="sxs-lookup"><span data-stu-id="a02dc-458">BaseNearInteractionTouchable.DistFront has been moved and renamed to PokePointer.TouchableDistance This is the distance and which the PokePointer can interact with touchables.</span></span> <span data-ttu-id="a02dc-459">先前每個可觸式都有自己的最大互動距離，但現在會在 PokePointer 中定義，以提供更佳的優化。</span><span class="sxs-lookup"><span data-stu-id="a02dc-459">Previously each touchable had its own maximum interaction distance, but now this is defined in the PokePointer which allows better optimization.</span></span>
- <span data-ttu-id="a02dc-460">BaseNearInteractionTouchable 已重新命名為 PokeThreshold，因此它會清楚指出 PokeThreshold 是 DebounceThreshold 的對應項。</span><span class="sxs-lookup"><span data-stu-id="a02dc-460">BaseNearInteractionTouchable.DistBack has been renamed to PokeThreshold This makes it clear that PokeThreshold is the counterpart to DebounceThreshold.</span></span> <span data-ttu-id="a02dc-461">當超過 PokeThreshold 時，即會啟動可觸式，並在超過 DebounceThreshold 時釋放。</span><span class="sxs-lookup"><span data-stu-id="a02dc-461">A touchable is activated when the PokeThreshold is crossed, and released when DebounceThreshold is crossed.</span></span>

<span data-ttu-id="a02dc-462">**ReadOnlyAttribute**</span><span class="sxs-lookup"><span data-stu-id="a02dc-462">**ReadOnlyAttribute**</span></span>

<span data-ttu-id="a02dc-463">已將 `Microsoft.MixedReality.Toolkit` 命名空間加入至 `ReadOnlyAttribute` 、 `BeginReadOnlyGroupAttribute` 和 `EndReadOnlyGroupAttribute` 。</span><span class="sxs-lookup"><span data-stu-id="a02dc-463">The `Microsoft.MixedReality.Toolkit` namespace has been added to `ReadOnlyAttribute`, `BeginReadOnlyGroupAttribute`, and `EndReadOnlyGroupAttribute`.</span></span>

<span data-ttu-id="a02dc-464">**PointerClickHandler**</span><span class="sxs-lookup"><span data-stu-id="a02dc-464">**PointerClickHandler**</span></span>

<span data-ttu-id="a02dc-465">`PointerClickHandler` 類別已被取代。</span><span class="sxs-lookup"><span data-stu-id="a02dc-465">The `PointerClickHandler` class has been deprecated.</span></span> <span data-ttu-id="a02dc-466">您 `PointerHandler` 應該改為使用，它會提供相同的功能。</span><span class="sxs-lookup"><span data-stu-id="a02dc-466">The `PointerHandler` should be used instead, it provides the same functionality.</span></span>

<span data-ttu-id="a02dc-467">**HoloLens clicker 支援**</span><span class="sxs-lookup"><span data-stu-id="a02dc-467">**HoloLens clicker support**</span></span>

<span data-ttu-id="a02dc-468">HoloLens clicker 的控制器對應已從狀況變更 [`WindowsMixedRealityController`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityController) 為狀況 [`WindowsMixedRealityGGVHand`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityGGVHand) 。</span><span class="sxs-lookup"><span data-stu-id="a02dc-468">The HoloLens clicker's controller mappings have changed from being an unhanded [`WindowsMixedRealityController`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityController) to being an unhanded [`WindowsMixedRealityGGVHand`](xref:Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input.WindowsMixedRealityGGVHand).</span></span> <span data-ttu-id="a02dc-469">為了因應此情況，當您第一次開啟 ControllerMapping 設定檔時，會執行自動更新程式。</span><span class="sxs-lookup"><span data-stu-id="a02dc-469">To account for this, an automatic updater will run the first time you open your ControllerMapping profile.</span></span> <span data-ttu-id="a02dc-470">請至少在升級至2.0.0 之後開啟任何自訂設定檔一次，以觸發這個一次性的遷移步驟。</span><span class="sxs-lookup"><span data-stu-id="a02dc-470">Please open any custom profiles at least once after upgrading to 2.0.0 in order to trigger this one-time migration step.</span></span>

<span data-ttu-id="a02dc-471">**InteractableHighlight**</span><span class="sxs-lookup"><span data-stu-id="a02dc-471">**InteractableHighlight**</span></span>

<span data-ttu-id="a02dc-472">`InteractableHighlight` 類別已被取代。</span><span class="sxs-lookup"><span data-stu-id="a02dc-472">The `InteractableHighlight` class has been deprecated.</span></span> <span data-ttu-id="a02dc-473">`InteractableOnFocus` `FocusInteractableStates` 應改為使用類別和資產。</span><span class="sxs-lookup"><span data-stu-id="a02dc-473">The `InteractableOnFocus` class and `FocusInteractableStates` asset should be used instead.</span></span> <span data-ttu-id="a02dc-474">若要為建立新的 `Theme` 資產 `InteractableOnFocus` ，請在 [專案] 視窗中按一下滑鼠右鍵，然後選取 [*建立*  >  *混合現實工具* 組  >  *互動*  >  *主題*]。</span><span class="sxs-lookup"><span data-stu-id="a02dc-474">To create a new `Theme` asset for the `InteractableOnFocus`, right click in the project window and select *Create* > *Mixed Reality Toolkit* > *Interactable* > *Theme*.</span></span>

<span data-ttu-id="a02dc-475">**HandInteractionPanZoom**</span><span class="sxs-lookup"><span data-stu-id="a02dc-475">**HandInteractionPanZoom**</span></span>

<span data-ttu-id="a02dc-476">`HandInteractionPanZoom` 已移至 UI 命名空間，因為它不是輸入元件。</span><span class="sxs-lookup"><span data-stu-id="a02dc-476">`HandInteractionPanZoom` has been moved to the UI namespace as it was not an input component.</span></span> <span data-ttu-id="a02dc-477">`HandPanEventData` 也已移至此命名空間，並簡化為與其他 UI 事件資料對應。</span><span class="sxs-lookup"><span data-stu-id="a02dc-477">`HandPanEventData` has also been moved into this namespace, and simplified to correspond with other UI event data.</span></span>

### <a name="assembly-name-changes-in-200"></a><span data-ttu-id="a02dc-478">2.0.0 中的元件名稱變更</span><span class="sxs-lookup"><span data-stu-id="a02dc-478">Assembly name changes in 2.0.0</span></span>

<span data-ttu-id="a02dc-479">在2.0.0 版本中，所有的官方混合現實工具組元件名稱及其相關聯的元件定義 (. asmdef) 檔案都已更新為符合下列模式。</span><span class="sxs-lookup"><span data-stu-id="a02dc-479">In The 2.0.0 release, all of the official Mixed Reality Toolkit assembly names and their associated assembly definition (.asmdef) files have been updated to fit the following pattern.</span></span>

```c#
Microsoft.MixedReality.Toolkit[.<name>]
```

<span data-ttu-id="a02dc-480">在某些情況下，已合併多個元件以建立更好的 unity 內容。</span><span class="sxs-lookup"><span data-stu-id="a02dc-480">In some instances, multiple assemblies have been merged to create better unity of their contents.</span></span> <span data-ttu-id="a02dc-481">如果您的專案使用自訂的 asmdef 檔案，則可能需要更新。</span><span class="sxs-lookup"><span data-stu-id="a02dc-481">If your project uses custom .asmdef files, they may require updating.</span></span>

<span data-ttu-id="a02dc-482">下表說明 RC2 asmdef 檔案名如何對應至2.0.0 版本。</span><span class="sxs-lookup"><span data-stu-id="a02dc-482">The following tables describe how the RC2 .asmdef file names map to the 2.0.0 release.</span></span> <span data-ttu-id="a02dc-483">所有元件名稱都符合 asmdef 檔案名。</span><span class="sxs-lookup"><span data-stu-id="a02dc-483">All assembly names match the .asmdef file name.</span></span>

<span data-ttu-id="a02dc-484">**MixedRealityToolkit**</span><span class="sxs-lookup"><span data-stu-id="a02dc-484">**MixedRealityToolkit**</span></span>

| <span data-ttu-id="a02dc-485">RC2</span><span class="sxs-lookup"><span data-stu-id="a02dc-485">RC2</span></span> | <span data-ttu-id="a02dc-486">2.0.0</span><span class="sxs-lookup"><span data-stu-id="a02dc-486">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="a02dc-487">MixedRealityToolkit.asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-487">MixedRealityToolkit.asmdef</span></span> | <span data-ttu-id="a02dc-488">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-488">Microsoft.MixedReality.Toolkit.asmdef</span></span> |
| <span data-ttu-id="a02dc-489">MixedRealityToolkit. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-489">MixedRealityToolkit.Core.BuildAndDeploy.asmdef</span></span> | <span data-ttu-id="a02dc-490">MixedReality. BuildAndDeploy. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-490">Microsoft.MixedReality.Toolkit.Editor.BuildAndDeploy.asmdef</span></span> |
| <span data-ttu-id="a02dc-491">MixedRealityToolkit，asmdef 的</span><span class="sxs-lookup"><span data-stu-id="a02dc-491">MixedRealityToolkit.Core.Definitions.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="a02dc-492">已移除，請使用 MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-492">Removed, use Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="a02dc-493">MixedRealityToolkit. EditorClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-493">MixedRealityToolkit.Core.Extensions.EditorClassExtensions.asmdef</span></span> | <span data-ttu-id="a02dc-494">MixedReality. ClassExtensions. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-494">Microsoft.MixedReality.Toolkit.Editor.ClassExtensions.asmdef</span></span>
| <span data-ttu-id="a02dc-495">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-495">MixedRealityToolkit.Core.Inspectors.asmdef</span></span> | <span data-ttu-id="a02dc-496">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-496">Microsoft.MixedReality.Toolkit.Editor.Inspectors.asmdef</span></span> |
| <span data-ttu-id="a02dc-497">MixedRealityToolkit ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-497">MixedRealityToolkit.Core.Inspectors.ServiceInspectors.asmdef</span></span> | <span data-ttu-id="a02dc-498">MixedReality. ServiceInspectors. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-498">Microsoft.MixedReality.Toolkit.Editor.ServiceInspectors.asmdef</span></span> |
| <span data-ttu-id="a02dc-499">MixedRealityToolkit. UtilitiesAsync. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-499">MixedRealityToolkit.Core.UtilitiesAsync.asmdef</span></span> | <span data-ttu-id="a02dc-500">MixedReality. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-500">Microsoft.MixedReality.Toolkit.Async.asmdef</span></span> |
| <span data-ttu-id="a02dc-501">Asmdef 的 MixedRealityToolkit。</span><span class="sxs-lookup"><span data-stu-id="a02dc-501">MixedRealityToolkit.Core.Utilities.Editor.asmdef</span></span> | <span data-ttu-id="a02dc-502">MixedReality 工具組。 asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-502">Microsoft.MixedReality.Toolkit.Editor.Utilities.asmdef</span></span> |
| <span data-ttu-id="a02dc-503">MixedRealityToolkit Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-503">MixedRealityToolkit.Utilities.Gltf.asmdef</span></span> | <span data-ttu-id="a02dc-504">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-504">Microsoft.MixedReality.Toolkit.Gltf.asmdef</span></span> |
| <span data-ttu-id="a02dc-505">MixedRealityToolkit，Gltf asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-505">MixedRealityToolkit.Utilities.Gltf.Importers.asmdef</span></span> | <span data-ttu-id="a02dc-506">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-506">Microsoft.MixedReality.Toolkit.Gltf.Importers.asmdef</span></span> |

<span data-ttu-id="a02dc-507">**MixedRealityToolkit。提供者**</span><span class="sxs-lookup"><span data-stu-id="a02dc-507">**MixedRealityToolkit.Providers**</span></span>

| <span data-ttu-id="a02dc-508">RC2</span><span class="sxs-lookup"><span data-stu-id="a02dc-508">RC2</span></span> | <span data-ttu-id="a02dc-509">2.0.0</span><span class="sxs-lookup"><span data-stu-id="a02dc-509">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="a02dc-510">MixedRealityToolkit. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-510">MixedRealityToolkit.Providers.OpenVR.asmdef</span></span> | <span data-ttu-id="a02dc-511">MixedReality. OpenVR. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-511">Microsoft.MixedReality.Toolkit.Providers.OpenVR.asmdef</span></span> |
| <span data-ttu-id="a02dc-512">MixedRealityToolkit. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-512">MixedRealityToolkit.Providers.WindowsMixedReality.asmdef</span></span> | <span data-ttu-id="a02dc-513">MixedReality. WindowsMixedReality. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-513">Microsoft.MixedReality.Toolkit.Providers.WindowsMixedReality.asmdef</span></span> |
| <span data-ttu-id="a02dc-514">MixedRealityToolkit. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-514">MixedRealityToolkit.Providers.WindowsVoiceInput.asmdef</span></span> | <span data-ttu-id="a02dc-515">MixedReality. WindowsVoiceInput. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-515">Microsoft.MixedReality.Toolkit.Providers.WindowsVoiceInput.asmdef</span></span> |

<span data-ttu-id="a02dc-516">**MixedRealityToolkit 服務**</span><span class="sxs-lookup"><span data-stu-id="a02dc-516">**MixedRealityToolkit.Services**</span></span>

| <span data-ttu-id="a02dc-517">RC2</span><span class="sxs-lookup"><span data-stu-id="a02dc-517">RC2</span></span> | <span data-ttu-id="a02dc-518">2.0.0</span><span class="sxs-lookup"><span data-stu-id="a02dc-518">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="a02dc-519">MixedRealityToolkit. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-519">MixedRealityToolkit.Services.BoundarySystem.asmdef</span></span> | <span data-ttu-id="a02dc-520">MixedReality. BoundarySystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-520">Microsoft.MixedReality.Toolkit.Services.BoundarySystem.asmdef</span></span> |
| <span data-ttu-id="a02dc-521">MixedRealityToolkit. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-521">MixedRealityToolkit.Services.CameraSystem.asmdef</span></span> | <span data-ttu-id="a02dc-522">MixedReality. CameraSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-522">Microsoft.MixedReality.Toolkit.Services.CameraSystem.asmdef</span></span> |
| <span data-ttu-id="a02dc-523">MixedRealityToolkit. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-523">MixedRealityToolkit.Services.DiagnosticsSystem.asmdef</span></span> | <span data-ttu-id="a02dc-524">MixedReality. DiagnosticsSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-524">Microsoft.MixedReality.Toolkit.Services.DiagnosticsSystem.asmdef</span></span> |
| <span data-ttu-id="a02dc-525">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-525">MixedRealityToolkit.Services.InputSimulation.asmdef</span></span> | <span data-ttu-id="a02dc-526">MixedReality. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-526">Microsoft.MixedReality.Toolkit.Services.InputSimulation.asmdef</span></span> |
| <span data-ttu-id="a02dc-527">MixedRealityToolkit. InputSimulation. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-527">MixedRealityToolkit.Services.InputSimulation.Editor.asmdef</span></span> | <span data-ttu-id="a02dc-528">MixedReality. InputSimulation. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-528">Microsoft.MixedReality.Toolkit.Services.InputSimulation.Editor.asmdef</span></span> |
| <span data-ttu-id="a02dc-529">MixedRealityToolkit. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-529">MixedRealityToolkit.Services.InputSystem.asmdef</span></span> | <span data-ttu-id="a02dc-530">MixedReality. InputSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-530">Microsoft.MixedReality.Toolkit.Services.InputSystem.asmdef</span></span> |
| <span data-ttu-id="a02dc-531">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-531">MixedRealityToolkit.Services.Inspectors.asmdef</span></span> | <span data-ttu-id="a02dc-532">MixedReality. InputSystem. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-532">Microsoft.MixedReality.Toolkit.Services.InputSystem.Editor.asmdef</span></span> |
| <span data-ttu-id="a02dc-533">MixedRealityToolkit. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-533">MixedRealityToolkit.Services.SceneSystem.asmdef</span></span> | <span data-ttu-id="a02dc-534">MixedReality. SceneSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-534">Microsoft.MixedReality.Toolkit.Services.SceneSystem.asmdef</span></span> |
| <span data-ttu-id="a02dc-535">MixedRealityToolkit. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-535">MixedRealityToolkit.Services.SpatialAwarenessSystem.asmdef</span></span> | <span data-ttu-id="a02dc-536">MixedReality. SpatialAwarenessSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-536">Microsoft.MixedReality.Toolkit.Services.SpatialAwarenessSystem.asmdef</span></span> |
| <span data-ttu-id="a02dc-537">MixedRealityToolkit. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-537">MixedRealityToolkit.Services.TeleportSystem.asmdef</span></span> | <span data-ttu-id="a02dc-538">MixedReality. TeleportSystem. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-538">Microsoft.MixedReality.Toolkit.Services.TeleportSystem.asmdef</span></span> |

<span data-ttu-id="a02dc-539">**MixedRealityToolkit SDK**</span><span class="sxs-lookup"><span data-stu-id="a02dc-539">**MixedRealityToolkit.SDK**</span></span>

| <span data-ttu-id="a02dc-540">RC2</span><span class="sxs-lookup"><span data-stu-id="a02dc-540">RC2</span></span> | <span data-ttu-id="a02dc-541">2.0.0</span><span class="sxs-lookup"><span data-stu-id="a02dc-541">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="a02dc-542">MixedRealityToolkit. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-542">MixedRealityToolkit.SDK.asmdef</span></span> | <span data-ttu-id="a02dc-543">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-543">Microsoft.MixedReality.Toolkit.SDK.asmdef</span></span> |
| <span data-ttu-id="a02dc-544">MixedRealityToolkit asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-544">MixedRealityToolkit.SDK.Inspectors.asmdef</span></span> | <span data-ttu-id="a02dc-545">MixedReality 工具組. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-545">Microsoft.MixedReality.Toolkit.SDK.Inspectors.asmdef</span></span> |

<span data-ttu-id="a02dc-546">**MixedRealityToolkit 範例**</span><span class="sxs-lookup"><span data-stu-id="a02dc-546">**MixedRealityToolkit.Examples**</span></span>

| <span data-ttu-id="a02dc-547">RC2</span><span class="sxs-lookup"><span data-stu-id="a02dc-547">RC2</span></span> | <span data-ttu-id="a02dc-548">2.0.0</span><span class="sxs-lookup"><span data-stu-id="a02dc-548">2.0.0</span></span> |
| --- | --- |
| <span data-ttu-id="a02dc-549">MixedRealityToolkit。例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-549">MixedRealityToolkit.Examples.asmdef</span></span> | <span data-ttu-id="a02dc-550">MixedReality，例如 asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-550">Microsoft.MixedReality.Toolkit.Examples.asmdef</span></span> |
| <span data-ttu-id="a02dc-551">MixedRealityToolkit. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-551">MixedRealityToolkit.Examples.Demos.Gltf.asmdef</span></span> | <span data-ttu-id="a02dc-552">MixedReality. Gltf. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-552">Microsoft.MixedReality.Toolkit.Demos.Gltf.asmdef</span></span> |
| <span data-ttu-id="a02dc-553">MixedRealityToolkit，例如 StandardShader asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-553">MixedRealityToolkit.Examples.Demos.StandardShader.Inspectors.asmdef</span></span> | <span data-ttu-id="a02dc-554">MixedReality. StandardShader. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-554">Microsoft.MixedReality.Toolkit.Demos.StandardShader.Inspectors.asmdef</span></span> |
| <span data-ttu-id="a02dc-555">MixedRealityToolkit，InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-555">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.asmdef</span></span> | <span data-ttu-id="a02dc-556">MixedReality. InspectorFields. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-556">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.asmdef</span></span> |
| <span data-ttu-id="a02dc-557">MixedRealityToolkit，例如 InspectorFields asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-557">MixedRealityToolkit.Examples.Demos.Utilities.InspectorFields.Inspectors.asmdef</span></span> | <span data-ttu-id="a02dc-558">MixedReality. InspectorFields. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-558">Microsoft.MixedReality.Toolkit.Demos.InspectorFields.Inspectors.asmdef</span></span> |
| <span data-ttu-id="a02dc-559">MixedRealityToolkit，例如 Interactables. asmdef</span><span class="sxs-lookup"><span data-stu-id="a02dc-559">MixedRealityToolkit.Examples.Demos.UX.Interactables.asmdef</span></span> | <span data-ttu-id="a02dc-560">MixedReality. Interactables. asmdef。</span><span class="sxs-lookup"><span data-stu-id="a02dc-560">Microsoft.MixedReality.Toolkit.Demos.UX.Interactables.asmdef</span></span> |
