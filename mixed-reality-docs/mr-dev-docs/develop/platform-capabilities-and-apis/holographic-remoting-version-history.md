...
## <a name="version-261-july-20-2021"></a>版本 2.6.1 (2021 年7月20日) <a name="v2.6.1"></a>
* XR_MSFT_holographic_remoting_speech 擴充功能現在允許在執行中會話期間，使用新參數重新初始化語音辨識器。
* 已修正在多個連線中，語音辨識可靠性降低的問題。
* 各種 bug 修正和穩定性改進。
...
* 使用 OpenXR API 的全像攝影遠端功能現在支援：
  ...
  * XR_MSFT_spatial_graph_bridge 擴充功能，可讓應用程式建立 XrSpace 控制碼，以追蹤其他 Windows Mixed Reality 裝置平臺程式庫或 api 的空間 Graph-節點。 不過，有一點要注意的是，XR_SPATIAL_GRAPH_NODE_TYPE_STATIC_MSFT 是 xrCreateSpatialGraphNodeSpaceMSFT 唯一支援的節點類型。 
* 使用 Mixed Reality API 的全像攝影遠端功能現在支援：
  * SpatialGraphInteropPreview. CreateCoordinateSystemForNode 多載，可讓應用程式追蹤靜態空間 Graph-節點，讓使用者可以在其環境中考慮位置和專案。
...
* 已修正使用 VR 控制器的觸發程式按鈕時的損毀。 全像是「全像」的遠端處理並不完全支援控制器，如果與 HoloLens 2 配對，只有 [觸發程式] 按鈕和 [Windows] 按鈕
