...
* 已修正造成視覺抖動的遠端姿勢預測的回歸。
* 已實 PerceptionDeviceSetCreateFactoryOverride，可確保在全像全像的遠端傳送的 PerceptionDevice.dll 不會干擾 Windows 10 隨附的版本。

## <a name="version-230-october-2-2020"></a>版本 2.3.0 (2020 年10月2日) <a name="v2.3.0"></a>
* 修正了全像攝影遠端播放程式暫停時可能發生的損毀。
* 穩定性改進。
...
* 您現在可以透過[SpatialInteractionSource](https://docs.microsoft.com/uwp/api/windows.ui.input.spatial.spatialinteractionsource.controller#Windows_UI_Input_Spatial_SpatialInteractionSource_Controller)來抓取全像遠端處理和控制器專屬資料支援的[動作控制器](../../design/motion-controllers.md)。
...
* 固定的空間錨點問題，非預期地關閉全像「全像」的播放機。 此問題也會影響自訂玩家。
...
* 修正了 SpatialGestureRecognizer 無法觸發暫停手勢事件的問題。
...
* 已修正使用 VR 控制器的觸發程式按鈕時的損毀。 全像「全像」的遠端處理不完全支援控制器，如果與 HoloLens 2 配對，則只有 [觸發程式] 按鈕和 [Windows] 按鈕可以運作
