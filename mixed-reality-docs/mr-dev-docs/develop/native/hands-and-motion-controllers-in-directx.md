...
* **控制器** 代表配對的移動控制器。 移動控制器可提供各種功能。  例如：選取觸發程式、功能表按鈕、抓住按鈕、touchpads 和 thumbsticks。
...
此按下的事件會以非同步方式傳送至您的應用程式，並在發生按下時與對應的 SpatialInteractionSourceState。 您的應用程式或遊戲引擎可能會想要立即執行一些處理，或者您可能想要將輸入處理常式中的事件資料排入佇列。 以下是 SourcePressed 事件的事件處理常式函式，它會顯示如何檢查是否已按下 [選取] 按鈕。
...
每個 SpatialInteractionSource 都有一個識別碼，可讓您用來識別新的來源，並將現有的來源與框架產生相互關聯。  每次離開並輸入 FOV 時，都會指派一個新的識別碼，但在會話期間，控制器識別碼仍保持靜態。  您可以使用 SpatialInteractionManager （例如 [SourceDetected](https://docs.microsoft.com//uwp/api/windows.ui.input.spatial.spatialinteractionmanager.sourcedetected) 和 [SourceLost](https://docs.microsoft.com//uwp/api/windows.ui.input.spatial.spatialinteractionmanager.sourcelost)）上的事件，以在進入或離開裝置的視圖時回應，或在動作控制器開啟/關閉或配對/不成對時做出反應。

### <a name="predicted-vs-historical-poses"></a>預測與歷程記錄
請注意，GetDetectedSourcesAtTimestamp 具有時間戳記參數。 這可讓您要求狀態，並提出預測或歷程記錄的資料，讓您可以將空間互動與其他輸入來源相互關聯。 例如，在目前的框架中呈現手的位置時，您可以傳入 [HolographicFrame](https://docs.microsoft.com//uwp/api/windows.graphics.holographic.holographicframe)所提供的預測時間戳記。 這可讓系統向前預測手的位置，以便與轉譯的畫面格輸出緊密對齊，並將察覺的延遲降至最低。

不過，這類預測的姿勢不會產生以互動來源為目標的理想指標光線。 例如，按下 [移動控制器] 按鈕時，最多可能需要20毫秒該事件，才能透過藍牙向上到作業系統。 同樣地，當使用者執行手勢手勢之後，可能會在系統偵測到手勢之前經過一段時間，然後您的應用程式會進行輪詢。 當您的應用程式輪詢狀態變更時，用來鎖定的 head 和手也會實際發生在過去。 如果您的目標是將目前的 HolographicFrame 時間戳記傳遞給 GetDetectedSourcesAtTimestamp，則會改為在畫面格顯示時，將姿勢預測到目標光線（未來可能超過20毫秒）。 這種未來的原因適用于 *呈現* 互動來源，但會將我們的時間問題轉譯為 *目標* ，因為使用者的目標是在過去發生。
...
請注意，兩個軸 (的板和操縱杆範圍是-1 到1，從下到上，以及從左至右) 。 使用 SpatialInteractionSourceState：： SelectPressedValue 屬性存取的類比觸發程式範圍，範圍為0到1。 值1與 IsSelectPressed 等於 true 的關聯性;任何其他值與 IsSelectPressed 的關聯性等於 false。
...
明確說明的手追蹤提供25個聯合基本架構，可啟用許多不同類型的互動。  此基本架構為索引/中間/環形/小手指提供5個接點、為 thumb 提供4個接點，以及1個手腕接點。  手腕聯合會作為階層的基礎。 下圖說明基本架構的版面配置。
...
除了25個階層式接點之外，系統也提供了一種掌上接點。  Palm 通常不會被視為框架結構的一部分。  它只是用來取得手的整體位置和方向的便利方法。
