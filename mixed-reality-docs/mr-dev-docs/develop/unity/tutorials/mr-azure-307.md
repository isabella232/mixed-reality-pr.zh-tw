...
8.  從 **分割資料項目** 的 ***左下方*** ，將連接拖曳至 **定型模型** 專案的 **右上** 方。 定型模型會使用資料集的前70% 分割來訓練演算法。
...
3.  接下來，移 **至 [** 檔案  >  **組建設定** ]，然後按一下 [ ***切換平臺*** ] 按鈕，將平臺切換至 **通用 Windows 平臺** 。
...
4.  回到 Unity 編輯器，確認 **ShelfKeeper** 類別看起來如下所示：
    ...
    > 2. **在階層** 中，以滑鼠左鍵按一下) 旁邊的箭號，以展開 **時間顯示** 物件 (。 接著，按一下階層內的 ***主要攝影機*** **，讓** 偵測 **器** 顯示其資訊。
...
4.  如果出現 [偵測 **到檔案修改** ] 對話方塊，請按一下 [ **重載方案** ]。
...
4.  在 [方案平臺] 中，選取 [ **x86** ]、[ **本機電腦** ]。 
    ...
    > - 知道 HoloLens 的 **IP 位址** ，您可以在 *設定 > Network & Internet > Wi-fi > Advanced Options* 中找到此位址;IPv4 是您應該使用的位址。 
