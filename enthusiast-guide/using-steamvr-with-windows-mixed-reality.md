...
如果您想要以較高的解析度播放遊戲，您隨時都可以在 [SteamVR-> 設定-> 應用程式] 視窗中調整應用程式解析度滑杆。 **請注意** ，當使用較高的解析度乘數時，您可能會預期遊戲會對您的電腦造成更多的壓力。 如果您增加乘數並看到降級的效能，請將此滑杆重新調整為預設層級並重新啟動遊戲，以確保變更生效。 ![調整應用程式解析度](images/SteamVR_Settings_Applications.png)
...
若要加入：
  ...
  3. 按一下 [ **Beta** ] 索引標籤。
  4. 選擇 **[Beta-public Beta]** ，然後按一下 [ **關閉** ] 以確認。 請注意，[Beta 版存取程式碼] 欄位應該保留空白。
...
若要加入：
  ...
  3. 按一下 [ **Beta** ] 索引標籤。
  4. 選擇 **[Beta-public Beta]** ，然後按一下 [ **關閉** ] 以確認。  請注意，[Beta 版存取程式碼] 欄位應該保留空白。 ![在 SteamVR 的 [屬性] 對話方塊中，切換至 SteamVR Beta](images/steamvr-beta.png)
...
4. 某些遊戲已回報為以50% 速度執行，或延遲)  (延遲增加。 請透過下列 [Windows 意見反應中樞](filing-feedback.md) 指示報告這些遊戲。
...
6. 如果您的意見反應是關於 SteamVR 效能，請收集混合現實效能追蹤： 
    ...
    3. 確定遊戲正在執行，然後按一下 [ **開始捕獲** ]。
    4. 花幾秒鐘的時間玩遊戲來捕捉追蹤。 請勿捕獲超過10-15 秒的追蹤，或太大而無法提交。
    5. 按一下 [ **停止捕捉** ]。
7. 當您完成其餘欄位之後，請按一下 [ **提交** ]。
