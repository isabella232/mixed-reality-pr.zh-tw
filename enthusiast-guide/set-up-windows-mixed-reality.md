...
如果您有沒有內建無線電的耳機，則必須將行動電話控制器與您的電腦配對， (在2018內建無線電) 之後製造的耳機。
...
不久之後，您的房間就會變成虛擬世界，讓您可以四處四處進行互動！ 請在您的房間內，為執行混合現實的空間提供並清除一些空間 (例如，清除一些樓層空間，然後將您的椅子移至房間) 的側邊。 我們建議您清除至少5英尺 x 7 英尺 (1.5 計量 x 2 計量) 空間，以找出混合的實際情況。
...
您可以在混合現實中啟用 Cortana 命令。 這可讓您在混合現實中使用語音命令來傳送、開啟應用程式，以及執行其他作業。 您將在 [學習混合實境](learn-mixed-reality.md) 章節中深入瞭解。

![混合的現實更適合使用語音](images/1050px-betterwithspeech.png)

## <a name="set-up-your-audio-headset"></a>設定音訊耳機

除非您購買了 Samsung HMD 電影對白 (，其中具有整合式 AKG 耳機和整合式雙麥克風陣列) ，所以您必須取得同時具有麥克風和) 耳機的音訊耳機 (，並將其插到耳機的 3.5 mm 音訊插孔中。 耳機的 3.5 mm 音訊插孔將視耳機型號而定-位於耳機面板的下方，或在耳機面板外的短音訊纜線結尾。
...
第二次進入混合的事實很容易，因為它會在連接到您的電腦時重新開啟耳機。 您也可以從 [開始] 功能表開啟混合實境入口的應用程式，藉以手動啟動該應用程式。 當您放置輸入和音訊時，輸入和音訊會自動路由傳送到耳機，您也可以按鍵盤上的 **Windows + Y** 手動觸發。
