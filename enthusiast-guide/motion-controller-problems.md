...
### <a name="what-do-the-vibrations-and-lights-mean"></a>振動和燈光是什麼意思？

LED constellation 環形和 haptics 指出移動控制器的狀態。

| State    | 與狀態相關聯的行為 | 如何進入/離開狀態 | 
|----------------------------|-----------------------------|----------------------------------------------------------------------|
...
4. 如果您的控制器已與您的電腦配對，請移至 [ **設定 > 裝置] > 藍牙 & 其他裝置** ，或如果您的控制器與耳機配對，請移至 **裝置管理員 > 運動控制器中的人體介面裝置** 。 請確定控制器列示為配對。 如果不是，請將 [它們配對](motion-controller-problems.md#how-do-i-pair-new-controllers-if-windows-mixed-reality-is-already-set-up-on-my-pc)。 
...
### <a name="my-controllers-jitter-get-stuck-or-flicker-and-disappear-in-mixed-reality"></a>我的控制器會抖動、停滯或閃爍，並且在混合現實中消失。 
...
### <a name="my-controller-is-stuck-in-an-infinite-reboot"></a>我的控制器卡在無限重新開機中。
...
### <a name="the-mixed-reality-portal-is-working-but-my-controllers-are-tracking-poorly-flying-away-shaking-etc"></a>混合實境入口可正常運作，但我的控制器在 (飛出、搖動等 ) 中追蹤不佳。
...
### <a name="my-motion-controllers-do-not-appear-in-steamvr-apps-and-games"></a>我的動畫控制器不會出現在 SteamVR apps 和遊戲中。
...
1. 開啟這兩個動作控制器。 如果您的控制器已與您的電腦配對，請移至 [ **設定 > 裝置] > 藍牙 & 其他裝置** ，或如果您的控制器與耳機配對，請移至 **裝置管理員 > 運動控制器中的人體介面裝置** 。 請確定它們顯示為「已連線」。 如果未顯示或顯示為「配對」，請將 [它們配對](https://docs.microsoft.com/windows/mixed-reality/enthusiast-guide/set-up-windows-mixed-reality)。
...
5. 按兩下每個「移動控制器」裝置，然後移至 [驅動程式] 索引標籤。確認列出的驅動程式版本對應到其中一個 [版本](https://docs.microsoft.com/windows/mixed-reality/enthusiast-guide/mixed-reality-software#mixed-reality-motion-controller-model-driver-release-history)。
6. 如果不是，請執行 Windows Update，這會自動下載並安裝驅動程式。 如果您是在具有企業原則的電腦上，或者如果 Windows Update 其他限制，則可能需要手動安裝動作控制器模型驅動程式。 若要這樣做，請造訪 [此頁面](https://docs.microsoft.com/windows/mixed-reality/enthusiast-guide/mixed-reality-software#mixed-reality-motion-controller-model-driver-release-history) ，並尋找與您的 Windows 10 版本對應的驅動程式版本。 您可以從下載頁面取得安裝指示。

### <a name="the-controller-firmware-update-takes-significantly-longer-than-two-minutes"></a>控制器固件更新所需的時間明顯超過兩分鐘。
...
8. 再次將控制器與耳機或電腦[配對](set-up-windows-mixed-reality.md#if-you-need-to-pair-your-motion-controllers-with-your-pc)。
9. 連接主機和耳機之後，裝置將會更新至最新的可用固件。

### <a name="can-i-pair-my-xbox-controller-to-my-pc-so-i-can-use-it-in-headset"></a>我可以將 Xbox 控制器與我的電腦配對，以便在耳機中使用嗎？
...
## <a name="if-your-motion-controllers-are-paired-to-your-headset"></a>如果您的運動控制器與耳機配對：

### <a name="should-i-pair-my-controllers-to-a-windows-mixed-reality-headset-that-has-built-in-bluetooth-radio"></a>我是否應該將控制器配對至有內建藍牙無線電的 Windows Mixed Reality 耳機？

某些 Windows Mixed Reality 耳機（包括 Acer OJO 500 和 Samsung 電影對白 +）有內建的藍牙無線電波可搭配運動控制器使用。 隨附于這些耳機的移動控制器會預先配對至原廠的耳機，不需要您的電腦具有個別的藍牙無線電。 您 _可以_ 手動將這些移動控制器與電腦的藍牙無線電配對，例如與沒有內建藍牙無線電的 Windows Mixed Reality 耳機搭配使用。 

### <a name="how-do-i-pair-new-controllers-if-windows-mixed-reality-is-already-set-up-on-my-pc"></a>如果電腦上已設定 Windows Mixed Reality，如何? 將新控制器配對？
...
### <a name="my-paired-controllers-dont-show-up-in-the-mixed-reality-portal"></a>我的配對控制器不會顯示在混合實境入口中。 

* 將控制器放在您的耳機前方，然後按下 [Windows] 按鈕四秒再重新開機，然後再按兩秒鐘。 
* 如果您在 **裝置管理員 > 的人類介面裝置** 中看到「移動控制器」，請再次流覽配對程式。 
* 如果控制器 Led 正在迴圈中，一次開啟一個光線象限，然後將它們關閉，則會進行固件更新。 等候更新完成，控制器應該會出現在混合的現實中。 
* 如果使用外部藍牙介面卡，請確定介面卡已插入 USB 2.0 埠 (通常（但不一定）是黑色) ，而不是從其他無線發送器或 USB 3.0 裝置。 
* 如果電腦剛剛損毀，且正在使用 Qualcomm 介面卡，則重設可能無法運作。 若要修正此問題，請從電腦背面拔下電源 (或者，如果是在膝上型電腦上，請按住電源按鈕10秒) 然後重新開機電腦。 
* 在 [設定] 中執行藍牙疑難排解員 **> 更新 & 安全性 > 疑難排解 > 藍牙** 。  

### <a name="how-can-i-return-my-controllers-to-their-factory-pairing"></a>如何將控制器恢復為其原廠配對？
...
## <a name="if-your-motion-controllers-are-paired-to-your-pc"></a>**如果您的移動控制器已與您的電腦配對：**

### <a name="my-motion-controllers-are-not-pairing"></a>我的動畫控制器未配對。 
...
* 將控制器放在您的耳機前方，然後按下 [Windows] 按鈕四秒再重新開機，然後再按兩秒鐘。 
* 如果您的控制器在 [設定] 中顯示為 [已連線] **> 藍牙 & 其他裝置** ，請將它們，然後再次執行配對程式。 
* 如果控制器 Led 正在迴圈中，一次開啟一個光線象限，然後將它們關閉，則會進行固件更新。 等候更新完成，控制器應該會出現在混合的現實中。 
...
1. 將兩個 AA 電池插入每個控制器。 不要讓電池蓋回。
2. 按住 [Windows] 按鈕兩秒鐘，以開啟每個控制器。 他們會在開啟時看到資訊。
3. 讓控制器進入配對模式。 [配對] 按鈕位於電池區間內 (查看此 [影像](set-up-windows-mixed-reality.md#if-you-need-to-pair-your-motion-controllers-with-your-pc)) 。 按住它，直到控制器燈開始閃爍為止。
4. 移至 [ **設定 > 裝置] > 藍牙 & 其他裝置** ，然後 **新增藍牙或其他裝置 > 藍牙** 。 當控制器出現時，請選取它們以配對。
...
如果您的電腦沒有藍牙，則建議使用的其中一個轉換器是 [PLUGABLE USB 藍牙4.0 低功耗微配接器](https://www.amazon.com/Plugable-Bluetooth-Adapter-Raspberry-Compatible/dp/B009ZIILLI/ref=sr-1-1?ie=UTF8&qid=1490148230&sr=8-1&keywords=plugable+broadcom)。

### <a name="wi-fi-slows-down-on-my-notebook-when-motion-controllers-are-turned-on"></a>開啟動作控制器時，wi-fi 會減緩筆記本的速度。

當連線至 2.4 GHz 存取點時，您的筆記本可以與藍牙共用其 Wi-fi 天線。 如果您可以將波段喜好設定切換為5GHz，請簽入裝置管理員。 如果無法使用5GHz 網路，而且效能受到嚴重的影響，請考慮使用藍牙轉換器。
