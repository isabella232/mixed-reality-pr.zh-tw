...
此設定 ( **設定 > 混合現實 > 耳機顯示 > 體驗選項** ) 可讓您變更 Windows Mixed Reality 效能設定。 這可讓您在各種內容中選擇硬體設定的最佳體驗。 這些選項如下：
...
如果您的電腦設定為不同的語言，您可以使用 Windows Mixed Reality，但介面會以英文 (顯示) ，而且語音命令和聽寫將無法使用。 Windows Mixed Reality 螢幕小鍵盤僅限英文 (美國) 。 若要以另一種語言輸入文字，請使用連接到您電腦的實體鍵盤。 您也可以使用上面所列的其中一種支援 Windows Mixed Reality 語言的聽寫，只要在螢幕小鍵盤上選取 [麥克風] 即可。
...
9. 在 Windows PowerShell： <ul>
   ...
   <li>複製開頭為類比的功能身分識別 (如果不存在，則表示未安裝這個專案。 在此情況下，請跳至步驟 10 ) 。</li> 
