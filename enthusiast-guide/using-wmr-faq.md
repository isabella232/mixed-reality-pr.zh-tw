...
Windows Mixed Reality 是設計用來在您的沉浸式耳機上播放音效，並將其連接到耳機。 當您離開耳機或翻轉面板時，音訊會切換至預設的 Windows 播放裝置。 您可以在 [ **設定 > 混合現實 > 音訊和語音**] 中變更此設定。
...
如果您的耳機沒有內建的 mic，請使用 mic 將一對耳機連接到您的耳機或個人電腦。 選取 **設定 > 混合現實 > 音訊和語音** ，在耳機連線時，自動將 mic 輸入切換到耳機。 請確定 **當我磨損耳機時，切換到耳機 mic** 已開啟。
...
您也可以暫時關閉混合實境入口：選取 **功能表** 中的界限，然後使用切換開關來關閉界限。 關閉界限時，您必須停留在單一位置，以避免阻礙。

## <a name="im-having-trouble-with-my-motion-controllers"></a>我的動作控制器發生問題

如果您的動作控制器未正常運作、未連線，或是當您佩戴耳機時看不到控制器的影像：
...
* 選取 [ **開始] > 混合實境入口** 在您的電腦上，然後選取 [ **功能表** ]，您應該會看到所列出的動作控制器，以及狀態訊息：
    ...
    * 電力偏低–更換控制器電池。
* 如果使用 Wi-fi，請嘗試將您的電腦連接到 5 GHz 的 Wi-Fi 網路，以減少無線干擾。 
...
* 適用于沒有內建藍牙可直接與控制器配對的舊耳機：  
    ...
    * 在 [設定] > 裝置中移除藍牙耳機和喇叭 **> 藍牙 & 其他裝置**，並關閉裝置。 
    * 如果您使用的是 USB Bluetooth 介面卡，請確定它已插入黑色 USB 2.0 埠。 也請確定已從任何其他無線發送器或 USB 快閃磁片磁碟機（包括耳機的 USB 連接器）將介面卡插到最遠的距離。 
...
* 請嘗試調整耳機的校正。 應調整鏡頭以符合您的 interpupillary 距離 (IPD) ，也就是瞳孔之間的距離。 如果您不知道 IPD，optometrist 可以為您進行測量。 也有設計來測量 IPD 的網站。 知道您的 IPD 之後，請使用耳機的校正旋鈕來進行調整。 如果耳機沒有校正旋鈕，請選取 [ **設定 > 混合現實 > 耳機顯示** 並調整校正控制項。
...
體驗選項設定 (**設定 > 混合現實 > 耳機顯示 > 體驗選項**) 讓您能夠變更 Windows Mixed Reality 效能設定。 這可讓您在各種內容中選擇硬體設定的最佳可能體驗。 90-Hz 體驗適用于所有系統，但您可能會想要先試用自動，以查看您偏好的設定。

選項如下：

* 自動或讓 Windows 決定： Windows Mixed Reality 將決定硬體設定的最佳體驗。 對於大部分的人來說，這是開始使用的最佳選擇。
* 60 Hz：將重新整理頻率設定為 60 Hz，並關閉特定功能，例如混合實境入口中的影片捕獲和預覽。
* 90 Hz：如果您的耳機能以該速度執行，請將重新整理頻率設定為 90 Hz。 如果纜線問題導致耳機無法在 90 Hz 執行，您可能會在啟動時看到此模式已選取時發生錯誤。 
